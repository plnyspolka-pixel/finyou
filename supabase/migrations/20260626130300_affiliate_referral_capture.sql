-- =====================================================================
-- Automatyczne przechwytywanie poleceń i emisja zdarzeń prowizyjnych.
--  - powiązanie użytkownika/wniosku z partnerem polecającym,
--  - trigger: wypłata środków klientowi → zdarzenie client_funds_disbursed,
--  - znacznik przetworzenia zdarzenia przez tick naliczający prowizje.
-- =====================================================================

-- 1. Partner polecający przypisany do użytkownika (first-touch).
...
-- 2. Partner polecający przypisany do wniosku (podstawa zdarzenia client_funds_disbursed).
...
-- 3. Znacznik przetworzenia zdarzenia (naliczenie prowizji sieciowej przez tick).
...
-- 3b. Zewnętrzny identyfikator płatności (np. Stripe session id — nie-UUID) + deduplikacja.
...
-- 4. Trigger: po wypłacie środków klientowi (status → 'wyplacony') twórz zdarzenie prowizyjne.
--    Bazą jest wynagrodzenie Finance You (ta sama formuła co w kalkulatorze: 10%→4% wg kwoty).
...
DECLARE
  ...
  v_t numeric;
BEGIN
  -- tylko realne przejście do statusu „wypłacony"
  IF NEW.status <> 'wyplacony' OR OLD.status IS NOT DISTINCT FROM NEW.status THEN
    RETURN NEW;
  END IF;

  v_partner := NEW.referred_by_partner_id;

  -- fallback 1: partner z profilu klienta
  ...
  -- fallback 2: przypisany operator, jeśli jest partnerem programu
  ...
  -- deduplikacja: jedno aktywne zdarzenie wypłaty na wniosek
  IF EXISTS (
    SELECT 1 FROM public.affiliate_commission_events
    WHERE application_id = NEW.id AND event_type = 'client_funds_disbursed'
      AND event_status NOT IN ('cancelled','refunded','chargeback','fraud')
  ) THEN
    RETURN NEW;
  END IF;
  ...
  -- zapis partnera na wniosku (audytowo), gdy pusty — nie zmienia statusu, więc bez rekurencji
