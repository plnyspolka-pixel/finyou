-- =====================================================================
-- PROGRAM POŚREDNIKÓW (sieć partnerska / 3-poziomowy system prowizji)
-- Prowizje wyłącznie od realnych zdarzeń gospodarczych (opłacone konta,
-- skutecznie rozliczeni klienci). Nigdy za samą rejestrację/zaproszenie.
-- =====================================================================
...
-- =====================================================================
-- 2. CLOSURE TABLE (struktura partnerska, przyspiesza wyszukiwanie)
--    depth 1 = bezpośrednio zaproszony, 2 = II poziom, 3 = III poziom.
-- =====================================================================
...
-- =====================================================================
-- 3. REGUŁY PROWIZJI (konfigurowalne w panelu admina — bez stawek na sztywno)
-- =====================================================================
CREATE TABLE IF NOT EXISTS public.affiliate_commission_rules (
  ...
  basis_type text NOT NULL CHECK (basis_type IN (
    'fixed_amount','percent_of_net_revenue','percent_of_gross_payment',
    'percent_of_loan_amount','percent_of_finance_you_fee','manual')),
...
-- =====================================================================
-- 4. LIMITY DZIAŁALNOŚCI NIEREJESTROWANEJ (edytowalne co rok)
-- =====================================================================
...
-- =====================================================================
-- 5. ZDARZENIA PROWIZYJNE (realne zdarzenia gospodarcze)
-- =====================================================================
CREATE TABLE IF NOT EXISTS public.affiliate_commission_events (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  event_type text NOT NULL CHECK (event_type IN ('investor_account_paid','broker_account_paid','client_funds_disbursed')),
  event_status text NOT NULL DEFAULT 'pending'
    CHECK (event_status IN ('pending','confirmed','refund_window','eligible','cancelled',
                            'refunded','chargeback','fraud','manually_blocked')),
  ...
  net_revenue_amount numeric,
  finance_you_fee_amount numeric,
  loan_amount numeric,
  currency text NOT NULL DEFAULT 'PLN',
  occurred_at timestamptz NOT NULL DEFAULT now(),
  confirmed_at timestamptz,
  refund_window_until timestamptz,
...
-- =====================================================================
-- 7. FAKTURY PARTNERÓW (B2B: faktura partnera za prowizję)
-- =====================================================================
CREATE TABLE IF NOT EXISTS public.affiliate_invoices (
  ...
  status text NOT NULL DEFAULT 'submitted'
    CHECK (status IN ('submitted','verified','rejected','paid')),
  created_at timestamptz NOT NULL DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS public.affiliate_commissions (
  ...
  status text NOT NULL DEFAULT 'calculated'
    CHECK (status IN ('calculated','pending_admin_approval','approved','blocked',
                      'requires_invoice','requires_unregistered_activity_statement',
                      'requires_business_registration','payable','paid','cancelled','clawback')),
  ...
  tax_quarter int NOT NULL CHECK (tax_quarter IN (1,2,3,4)),
...
CREATE TABLE IF NOT EXISTS public.affiliate_payout_items (
  ...
  transfer_title text,
  bank_account_snapshot_encrypted text,
...
-- ---------------------------------------------------------------------
-- Helper: id partnera zalogowanego użytkownika (SECURITY DEFINER omija RLS,
-- dzięki czemu można go bezpiecznie użyć w politykach tabel potomnych).
-- Definiowany po utworzeniu tabeli (funkcja SQL waliduje ciało przy tworzeniu).
-- ---------------------------------------------------------------------
...
-- =====================================================================
-- GRANTS (RLS poniżej ogranicza dostęp wierszowo)
-- =====================================================================
DO $$
DECLARE t text;
BEGIN
  FOREACH t IN ARRAY ARRAY[
    'affiliate_partners','affiliate_network_closure','affiliate_commission_rules',
    'affiliate_unregistered_activity_limits','affiliate_commission_events',
    'affiliate_commissions','affiliate_payout_batches','affiliate_payout_items',
    'affiliate_invoices','affiliate_audit_logs'
  ] LOOP
...
-- ---------------------------------------------------------------------
-- RLS: PARTNERZY
--  - partner widzi/edytuje WYŁĄCZNIE swój rekord (dane wrażliwe innych ukryte;
--    podgląd struktury idzie przez funkcję serwerową zwracającą dane zanonimizowane)
--  - admin/operator: pełny dostęp; księgowość: odczyt
-- ---------------------------------------------------------------------
...
-- Edycja danych partnera (status, sponsor, dane rozliczeniowe) odbywa się przez
-- funkcje serwerowe (service role) z białą listą pól — brak polityki UPDATE dla
-- partnera, aby nie mógł sam zmienić statusu ani sponsora.
CREATE POLICY "partners_admin_all" ON public.affiliate_partners FOR ALL TO authenticated
  USING (public.has_role(auth.uid(),'administrator'))
  WITH CHECK (public.has_role(auth.uid(),'administrator'));

-- CLOSURE: tylko personel (struktura partnera idzie przez funkcję serwerową)
...
-- REGUŁY: odczyt dla zalogowanych (transparentność), zapis tylko admin
...
-- LIMITY: odczyt dla zalogowanych, zapis admin
...
-- ZDARZENIA: partner widzi własne (bezpośrednie), personel wszystkie
CREATE POLICY "events_partner_select" ON public.affiliate_commission_events FOR SELECT TO authenticated
  USING (direct_partner_id = public.affiliate_current_partner_id()
         OR public.has_role(auth.uid(),'administrator')
         OR public.has_role(auth.uid(),'operator')
         OR public.has_role(auth.uid(),'ksiegowosc'));
...
-- POZYCJE WYPŁAT: partner widzi swoje, personel wszystkie
...
-- FAKTURY: partner widzi/dodaje swoje, personel wszystkie
...
-- AUDYT: tylko odczyt dla personelu (zapis przez service_role)
CREATE POLICY "affiliate_audit_staff_select" ON public.affiliate_audit_logs FOR SELECT TO authenticated
  USING (public.has_role(auth.uid(),'administrator') OR public.has_role(auth.uid(),'ksiegowosc'));

-- =====================================================================
-- SEED: limity działalności nierejestrowanej 2026 (edytowalne w panelu)
-- 10 813,50 zł = 225% minimalnego wynagrodzenia 4 806 zł
-- =====================================================================
INSERT INTO public.affiliate_unregistered_activity_limits (year, quarter, limit_amount, currency, source_note, active)
VALUES
...
-- =====================================================================
-- SEED: domyślne reguły prowizji (DO EDYCJI w panelu admina — wartości przykładowe)
-- =====================================================================
INSERT INTO public.affiliate_commission_rules (name, active, event_type, network_level, basis_type, fixed_amount, percent_rate, requires_admin_approval, refund_window_days)
VALUES
