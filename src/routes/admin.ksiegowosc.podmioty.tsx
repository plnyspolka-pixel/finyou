...
type EntityForm = {
  id?: string;
  name: string; legal_name: string; nip: string; regon: string;
...
function PodmiotyPage() {
  ...
  function edit(e: any) {
    setForm({
      id: e.id, name: e.name ?? "", legal_name: e.legal_name ?? "", nip: e.nip ?? "", regon: e.regon ?? "",
  ...
  async function save() {
    if (!form.name.trim() || !form.legal_name.trim()) return toast.error("Podaj nazwę i pełną nazwę podmiotu");
    setSaving(true);
    try {
      await upsertFn({ data: {
        id: form.id, name: form.name, legal_name: form.legal_name, nip: form.nip || null, regon: form.regon || null,
  ...
  async function makeDefault(id: string) {
    try { await defaultFn({ data: { id } }); toast.success("Ustawiono podmiot domyślny"); void qc.invalidateQueries({ queryKey: ["accounting-entities"] }); }
          ...
          <Card key={e.id} className={e.is_default ? "border-primary/40" : undefined}>
            <CardHeader className="pb-2">
              <CardTitle className="text-base flex items-center justify-between gap-2">
                <span className="flex items-center gap-2">{e.name} {e.is_default && <Badge variant="secondary" className="gap-1 bg-primary/10 text-primary"><Star className="h-3 w-3" /> domyślny</Badge>}</span>
                {!e.active && <Badge variant="secondary" className="bg-slate-200 text-slate-600">nieaktywny</Badge>}
              </CardTitle>
              <CardDescription>{e.legal_name}{e.nip ? ` · NIP ${e.nip}` : ""}</CardDescription>
            </CardHeader>
            <CardContent className="space-y-2 text-sm">
              ...
              <div className="flex gap-2 pt-2">
                ...
                {!e.is_default && <Button size="sm" variant="ghost" onClick={() => makeDefault(e.id)}><Star className="mr-2 h-3.5 w-3.5" /> Ustaw domyślny</Button>}
              </div>
            </CardContent>
          </Card>
        ))}
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-h-[85vh] overflow-y-auto">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            ...
            <div className="space-y-1"><Label>NIP</Label><Input value={form.nip} onChange={(e) => set("nip")(e.target.value)} /></div>
            <div className="space-y-1"><Label>REGON</Label><Input value={form.regon} onChange={(e) => set("regon")(e.target.value)} /></div>
            <div className="space-y-1 sm:col-span-2"><Label>Ulica i numer</Label><Input value={form.address_street} onChange={(e) => set("address_street")(e.target.value)} /></div>
            <div className="space-y-1"><Label>Kod pocztowy</Label><Input value={form.address_postal_code} onChange={(e) => set("address_postal_code")(e.target.value)} /></div>
            <div className="space-y-1"><Label>Miejscowość</Label><Input value={form.address_city} onChange={(e) => set("address_city")(e.target.value)} /></div>
            <div className="space-y-1 sm:col-span-2"><Label>Rachunek bankowy (na fakturze)</Label><Input value={form.bank_account} onChange={(e) => set("bank_account")(e.target.value)} /></div>
            <div className="space-y-1"><Label>Prefiks numeracji</Label><Input value={form.invoice_prefix} onChange={(e) => set("invoice_prefix")(e.target.value)} /></div>
            <div className="space-y-1"><Label>Domyślna stawka VAT</Label>
            ...
            <div className="space-y-1 sm:col-span-2"><Label>Sposób wystawiania faktur</Label>
              ...
              <>
                <div className="space-y-1"><Label>Środowisko KSeF</Label>
                  <Select value={form.ksef_environment} onValueChange={set("ksef_environment")}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>{Object.entries(KSEF_LABELS).map(([k, v]) => <SelectItem key={k} value={k}>{v}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
            ...
            <div className="flex items-center gap-2"><Switch checked={form.active} onCheckedChange={set("active")} /><Label>Aktywny</Label></div>
          </div>
          <DialogFooter><Button onClick={save} disabled={saving}>{saving ? "Zapisywanie…" : "Zapisz"}</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
  );
