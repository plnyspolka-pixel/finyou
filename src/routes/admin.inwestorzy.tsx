...
function InwestorzyAdmin() {
  ...
  const [form, setForm] = useState({ first_name: "", last_name: "", company_name: "", email: "", phone: "", investor_type: "indywidualny", subscription_plan: "podstawowy", subscription_status: "aktywny" });
  ...
  const add = async () => {
    const { error } = await supabase.from("investors").insert({
      first_name: form.first_name || null, last_name: form.last_name || null, company_name: form.company_name || null,
      email: form.email || null, phone: form.phone || null, investor_type: form.investor_type as any,
      subscription_plan: form.subscription_plan as any, subscription_status: form.subscription_status as any,
    });
    if (error) { toast.error(error.message); return; }
    toast.success("Dodano inwestora"); setOpen(false); void load();
  };

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold">Inwestorzy</h1><p className="text-sm text-muted-foreground">CRUD inwestorów indywidualnych i instytucjonalnych.</p></div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild><Button><Plus className="mr-2 h-4 w-4" />Dodaj inwestora</Button></DialogTrigger>
          <DialogContent>
            <DialogHeader><DialogTitle>Nowy inwestor</DialogTitle></DialogHeader>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div><Label>Plan</Label>
                <Select value={form.subscription_plan} onValueChange={(v) => setForm({ ...form, subscription_plan: v })}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>{Object.entries(subscriptionPlanLabels).map(([k, v]) => <SelectItem key={k} value={k}>{v}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <div><Label>Status abonamentu</Label>
                <Select value={form.subscription_status} onValueChange={(v) => setForm({ ...form, subscription_status: v })}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>{Object.entries(subscriptionStatusLabels).map(([k, v]) => <SelectItem key={k} value={k}>{v}</SelectItem>)}</SelectContent>
                </Select>
              </div>
            </div>
      ...
      <Card><CardHeader><CardTitle>Lista ({rows.length})</CardTitle></CardHeader>
        <CardContent><div className="overflow-x-auto"><Table>
          <TableHeader><TableRow><TableHead>Inwestor</TableHead><TableHead>Typ</TableHead><TableHead>Kontakt</TableHead><TableHead>Plan</TableHead><TableHead>Abonament</TableHead><TableHead></TableHead></TableRow></TableHeader>
          <TableBody>{rows.map((r) => (
            <TableRow key={r.id}>
              <TableCell className="font-medium">{r.company_name || `${r.first_name ?? ""} ${r.last_name ?? ""}`.trim() || "—"}</TableCell>
              <TableCell><Badge variant="outline">{investorTypeLabels[r.investor_type]}</Badge></TableCell>
