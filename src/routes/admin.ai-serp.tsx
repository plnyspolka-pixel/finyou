...
import { Search, Loader2, Trash2, RefreshCw, TrendingUp, TrendingDown, Minus, History } from "lucide-react";
import {
  addKeyword, deleteKeyword, updateKeyword,
  checkKeywordRanking, checkAllRankings,
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs defaultValue="keywords">
        ...
        <TabsContent value="keywords" className="mt-4"><KeywordList /></TabsContent>
        <TabsContent value="add" className="mt-4"><AddKeywordForm /></TabsContent>
      </Tabs>
    </div>
  );
}
...
function PositionDelta({ current, previous }: { current: number | null; previous: number | null }) {
  ...
  if (diff === 0) return (
    <span className="inline-flex items-center gap-1"><Badge variant="secondary">{current}</Badge><Minus className="h-3 w-3 text-muted-foreground" /></span>
  );
  return (
    <span className="inline-flex items-center gap-1">
      <Badge variant="secondary">{current}</Badge>
      {diff > 0
        ? <span className="inline-flex items-center text-emerald-600 text-xs"><TrendingUp className="h-3 w-3" />+{diff}</span>
        : <span className="inline-flex items-center text-red-600 text-xs"><TrendingDown className="h-3 w-3" />{diff}</span>}
    </span>
  );
}
...
function KeywordList() {
  ...
  useEffect(() => {
    (async () => {
      const { data: kws } = await supabase.from("ai_serp_keywords").select("*").order("created_at", { ascending: false });
      setItems(kws ?? []);
      if (kws?.length) {
        const { data: rks } = await supabase
          .from("ai_serp_rankings").select("*")
  ...
  const avg = ranked.length ? Math.round(ranked.reduce((s: number, r: any) => s + r.position, 0) / ranked.length) : 0;
    ...
    <div className="space-y-4">
      ...
      <div className="flex justify-end">
        <Button onClick={handleCheckAll} disabled={bulkBusy || !items.length}>
          {bulkBusy ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <RefreshCw className="mr-2 h-4 w-4" />}
          Sprawdź wszystkie
        </Button>
      </div>
        ...
        <CardContent className="p-0">
          <Table>
            ...
            <TableBody>
                ...
                return (
                  <TableRow key={k.id}>
                    <TableCell>
                      ...
                      <div className="text-xs text-muted-foreground">{k.location} · {k.language}{k.intent ? ` · ${k.intent}` : ""}</div>
                    </TableCell>
                    ...
                    <TableCell className="text-right">
                      <div className="inline-flex gap-1">
                        <Button size="sm" variant="ghost" onClick={() => handleCheck(k.id)} disabled={busy === k.id}>
                          {busy === k.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <RefreshCw className="h-4 w-4" />}
                        </Button>
                        <HistoryDialog keywordId={k.id} keyword={k.keyword} />
                        <Button size="sm" variant="ghost" onClick={async () => {
                          await upd({ data: { id: k.id, is_active: !k.is_active } });
                          reload();
                        }}>{k.is_active ? "Pauza" : "Aktywuj"}</Button>
                        <Button size="sm" variant="ghost" onClick={async () => {
              ...
              {!items.length && (
                <TableRow><TableCell colSpan={5} className="text-center text-muted-foreground py-6">Brak fraz. Dodaj pierwszą.</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </CardContent>
...
function HistoryDialog({ keywordId, keyword }: { keywordId: string; keyword: string }) {
  ...
  useEffect(() => {
    if (!open) return;
    supabase.from("ai_serp_rankings").select("*").eq("keyword_id", keywordId)
      .order("checked_at", { ascending: false }).limit(50).then(({ data }) => setRows(data ?? []));
  }, [open, keywordId]);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader><DialogTitle>Historia: {keyword}</DialogTitle></DialogHeader>
        <div className="max-h-[60vh] overflow-y-auto">
          <Table>
            <TableHeader><TableRow><TableHead>Data</TableHead><TableHead>Pozycja</TableHead><TableHead>URL</TableHead></TableRow></TableHeader>
            <TableBody>
              {rows.map((r) => (
                <TableRow key={r.id}>
              ...
              {!rows.length && <TableRow><TableCell colSpan={3} className="text-center text-muted-foreground py-4">Brak pomiarów.</TableCell></TableRow>}
            </TableBody>
          </Table>
        </div>
      </DialogContent>
...
function AddKeywordForm() {
  ...
  const [form, setForm] = useState({ keyword: "", target_url: "", location: "Poland", language: "pl", tags: "" });
      ...
      <CardContent>
        <form onSubmit={submit} className="space-y-4">
          <div>
            ...
            <Input value={form.keyword} onChange={(e) => setForm({ ...form, keyword: e.target.value })} required placeholder="np. pożyczka pod zastaw nieruchomości" />
          </div>
          <div>
            <Label>URL docelowy</Label>
            <Input value={form.target_url} onChange={(e) => setForm({ ...form, target_url: e.target.value })} placeholder="https://twojadomena.pl/strona" />
            <p className="text-xs text-muted-foreground mt-1">Pozostaw puste, aby tylko śledzić TOP 10 bez konkretnego URL.</p>
          </div>
          ...
          <div>
            ...
            <Input value={form.tags} onChange={(e) => setForm({ ...form, tags: e.target.value })} placeholder="brand, pożyczki, hipoteczne" />
          </div>
