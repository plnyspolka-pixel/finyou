...
const FLOW = [
  { t: "Klient lub pośrednik dodaje sprawę", d: "Numer KW i podstawowe informacje o nieruchomości." },
...
function Hero() {
  return (
    <section className="fy-hero" style={{ color: "#fff", borderBottom: "1px solid var(--border)" }}>
      ...
      <div className="fy-hero-grid" style={{ position: "relative", maxWidth: "80rem", margin: "0 auto", padding: "4.5rem 1.5rem 5rem", alignItems: "start" }}>
        <div>
          <span
            style={{
              display: "inline-flex",
          ...
          <h1 style={{ marginTop: "1.3rem", fontSize: "clamp(2.2rem, 4.4vw, 3.4rem)", fontWeight: 900, lineHeight: 1.05, letterSpacing: "-0.025em" }}>
          ...
          <p style={{ marginTop: "1.1rem", maxWidth: "34rem", fontSize: "1.05rem", lineHeight: 1.6, color: "rgba(255,255,255,.82)" }}>
            Finance You łączy klientów szukających finansowania, prywatnych inwestorów i pośredników w jednym systemie: z CRM-em, AI, dokumentacją, szkoleniami i automatyzacją procesu.
          </p>
          <div style={{ marginTop: "1.8rem", display: "flex", gap: "0.7rem", flexWrap: "wrap", alignItems: "center" }}>
          ...
          <div style={{ marginTop: "2rem", display: "flex", gap: "1.6rem", flexWrap: "wrap", fontSize: "0.8rem", color: "rgba(255,255,255,.7)" }}>
  ...
  return (
    <MarketingShell page="home">
      ...
      <Section id="sciezki">
        ...
        />
        <div className="fy-steps" style={{ marginTop: "2.5rem", display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "1.1rem" }}>
          {ROLES.map((r) => (
      ...
      <Section id="blog" tint>
        ...
        />
        <div style={{ marginTop: "2.5rem", borderRadius: "var(--radius-2xl)", overflow: "hidden", border: "1px solid var(--border)" }}>
          <iframe
