...
export const Route = createFileRoute("/blog/")({
  loader: async () => {
    const { data } = await supabase
      ...
      .select("id,slug,title,excerpt,reading_minutes,published_at,cover_image_url,cover_image_alt,primary_keyword,audience")
      .eq("status", "published")
...
function ArticleCard({ a }: { a: ArticleRow }) {
  return (
    ...
    >
      {a.cover_image_url && <BlogCover src={a.cover_image_url} alt={a.cover_image_alt || a.title} />}
      <div style={{ display: "flex", flexDirection: "column", flex: 1, padding: "1.25rem 1.3rem 1.2rem", gap: "0.6rem" }}>
        <div>{audienceBadge(a.audience)}</div>
        <h2 style={{ margin: 0, fontSize: "1.08rem", fontWeight: 700, lineHeight: 1.35, color: "var(--foreground)" }}>{a.title}</h2>
        {a.excerpt && (
          <p
            style={{
              margin: 0,
        ...
        <div style={{ marginTop: "auto", paddingTop: "0.4rem", fontSize: "0.74rem", color: "var(--muted-foreground)" }}>
  ...
  return (
    <MarketingShell page="blog">
      <section className="fy-hero" style={{ color: "#fff", borderBottom: "1px solid var(--border)" }}>
        ...
        <div style={{ position: "relative", maxWidth: "80rem", margin: "0 auto", padding: "3.5rem 1.5rem 3.8rem", textAlign: "center" }}>
          <Eyebrow tone="gold" style={{ letterSpacing: "0.24em" }}>
            Blog Finance You
          </Eyebrow>
          <h1 style={{ marginTop: "0.8rem", fontSize: "clamp(2rem, 3.6vw, 2.9rem)", fontWeight: 900, lineHeight: 1.08, letterSpacing: "-0.025em" }}>
          ...
          <p style={{ margin: "1.1rem auto 0", maxWidth: "44rem", fontSize: "1.02rem", lineHeight: 1.65, color: "rgba(255,255,255,.82)" }}>
            Porady i analizy dla właścicieli nieruchomości szukających{" "}
            <a href="/dla-klienta" style={{ color: "#a9cffb" }}>finansowania pod zabezpieczenie nieruchomości</a>{" "}
            oraz dla{" "}
            <a href="/dla-inwestora" style={{ color: "#a9cffb" }}>inwestorów w pożyczki hipoteczne</a>.
            Sprawdź też <a href="/oferty" style={{ color: "#a9cffb" }}>aktualne oferty pożyczek</a>.
          </p>
        </div>
      </section>

      <Section>
        <div className="fy-grid" style={{ display: "grid", gap: "1.2rem", gridTemplateColumns: "repeat(3, 1fr)" }}>
          {articles.map((a: ArticleRow) => (
            <ArticleCard key={a.id} a={a} />
          ))}
        </div>
        {articles.length === 0 && (
          <p style={{ textAlign: "center", color: "var(--muted-foreground)", padding: "3rem 0" }}>Brak artykułów.</p>
        )}
      </Section>
