...
type Row = {
  ...
  comms: {
    ...
    inboundAttachments?: { name: string }[];
  };
};
...
function filesCount(r: Row): number {
  const photos = (r.loan?.properties ?? []).reduce(
...
function KlienciPage() {
  ...
  const visibleRows = useMemo(() => {
    switch (quick) {
      case "nieobsluzone":
        return rows.filter((r) => r.status === "nowy");
      case "bez_kontaktu":
        return rows.filter((r) => !r.comms.lastAt);
      default:
        return rows;
    }
  }, [rows, quick]);
  ...
  const exportCsv = () => {
    ...
    const lines = rows.map((r) =>
      [
        ...
        r.loan?.loan_amount ?? "",
        r.loan?.preferred_period_months ?? "",
        r.loan?.completeness_percent ?? "",
        r.comms.calls,
        r.comms.sms,
        r.comms.emails,
        r.comms.lastAt ?? "",
    ...
    a.href = url;
    a.download = `klienci-${Date.now()}.csv`;
    a.click();
      ...
      <header className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 sm:flex sm:flex-wrap sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="text-xs sm:text-sm text-muted-foreground">
            Leady, wnioski, follow-up i przypomnienia voicebota — wszystko w jednym miejscu.
          </p>
        </div>
          ...
          <Card className="p-3 space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">
              ...
              <Select value={source} onValueChange={setSource}>
                <SelectTrigger className="h-9 text-xs col-span-2 sm:col-auto">
          ...
          <Card>
            ...
            <div className="lg:hidden divide-y">
              ...
              {!q.isLoading && visibleRows.length === 0 && (
                <div className="p-6 text-center text-sm text-muted-foreground">Brak rekordów.</div>
              )}
              {visibleRows.map((r) => {
                    ...
                    >
                      <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-start gap-2">
                        ...
                        <div className="flex shrink-0 flex-col items-end gap-1">
                          ...
                          <Badge
                            variant={r.type === "inwestorski" ? "secondary" : "default"}
                            className="text-[10px]"
                          >
                            {r.type}
                          </Badge>
                          ...
                          {isApplication(r) && r.loan && (
                            <div className="min-w-0">
                              ...
                              <div className="truncate font-medium">
                                {formatPLN(r.loan.loan_amount)}
                                {r.loan.preferred_period_months
                                  ? ` · ${r.loan.preferred_period_months} mc`
                                  : ""}
                              </div>
                              <div className="text-muted-foreground">
                                Kompletność: {r.loan.completeness_percent ?? 0}%
                              </div>
                            </div>
                          )}
                          {p && (
                      ...
                      <div className="mt-2 flex flex-wrap items-center gap-1.5 text-[11px] pl-6">
                        ...
                        {filesCount(r) > 0 && (
                          <span
                            className="inline-flex items-center gap-1 rounded-md bg-primary/10 text-primary px-1.5 py-0.5"
                            title="Pliki klienta"
                          >
                        ...
                        >
                              ...
                              <>
                                ...
                                <div className="text-muted-foreground">
                                  {r.loan.preferred_period_months
                                    ? `${r.loan.preferred_period_months} mc`
                                    : "—"}{" "}
                                  · {r.loan.completeness_percent ?? 0}%
                                </div>
                              </>
                            ) : (
                              <span className="text-muted-foreground">
                                lead{r.current_form_step ? ` · krok ${r.current_form_step}` : ""}
                              </span>
                            )}
                          </td>
                          <td className="px-3 py-2 text-xs">
                            {p ? (
                              <>
                                ...
                                <div className="text-muted-foreground">
                                  {p.city ?? "—"}
                                  {p.estimated_value ? ` · ${formatPLN(p.estimated_value)}` : ""}
                                </div>
                              </>
                            ) : (
                              "—"
                          ...
                          <td className="px-3 py-2 text-xs">
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              <span className="inline-flex items-center gap-1" title="Messenger/IG">
                                <MessagesSquare className="h-3 w-3" />
                                {r.comms.messenger ?? 0}
                              </span>
                              {r.comms.notes > 0 && (
                                <span className="inline-flex items-center gap-1" title="Notatki">
                                  <StickyNote className="h-3 w-3" />
                                  {r.comms.notes}
                                </span>
                              )}
                              {filesCount(r) > 0 && (
                                <span
                                  className="inline-flex items-center gap-1 rounded bg-primary/10 text-primary px-1.5 py-0.5"
                                  title="Pliki klienta"
                                >
                                  <Paperclip className="h-3 w-3" />
                                  {filesCount(r)}
                          ...
                          <td className="px-3 py-2 text-xs" title={r.source ?? undefined}>
                            {leadSourceLabel(r.source)}
                          </td>
