...
function InwestorOferty() {
  ...
  useEffect(() => { if (!user) return; void (async () => {
    ...
    const { data } = await supabase
      ...
      .select("*, loan_application:loan_applications(id, is_startup, startup_funding_dependency, business_status, business_legal_form, nip, business_nip_verified_at, investor_purpose, client:clients(bik_report_uploaded_at, bank_account_verified_at, phone_verified_at))")
