...
import {
  listCampaigns, createCampaign, deleteCampaign, updateCampaign, getCampaignStats,
  ...
  return (
    <div className="space-y-6">
      <header className="flex items-center justify-between">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button><Plus className="h-4 w-4 mr-2" />Nowa kampania</Button>
          </DialogTrigger>
          <DialogContent className="max-w-xl">
            <DialogHeader><DialogTitle>Nowa kampania</DialogTitle></DialogHeader>
            <NewCampaignForm onDone={() => { setOpen(false); void refresh(); }} />
          </DialogContent>
        </Dialog>
      </header>
      ...
      <div className="grid gap-3 md:grid-cols-4">
        ...
        <StatCard label="Suma kliknięć" value={campaigns.reduce((s, c) => s + (c.clicks || 0), 0)} />
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Krótki link działa pod <code>{baseUrl}/r/&lt;kod&gt;</code></CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
            <div className="grid place-items-center py-8 text-muted-foreground"><Loader2 className="h-5 w-5 animate-spin" /></div>
          ) : campaigns.length === 0 ? (
            <div className="text-sm text-muted-foreground py-6 text-center">Brak kampanii. Utwórz pierwszą.</div>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {campaigns.map((c) => {
                  ...
                  const cpl = c.leads && c.cost ? (Number(c.cost) / c.leads).toFixed(2) + " zł" : "—";
                  return (
                    <TableRow key={c.id}>
                      <TableCell>
                        <div className="font-medium">{c.name}</div>
                        <div className="text-xs text-muted-foreground truncate max-w-xs">{c.target_url}</div>
                      </TableCell>
                      <TableCell>
                        <div className="flex items-center gap-1">
                          <code className="text-xs">/r/{c.short_code}</code>
                          <Button size="icon" variant="ghost" className="h-6 w-6"
                            onClick={() => { void navigator.clipboard.writeText(shortLink); toast.success("Skopiowano"); }}>
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-1">
                          <Badge variant={c.is_active ? "default" : "secondary"} className="cursor-pointer"
                            onClick={async () => { await toggle({ data: { id: c.id, is_active: !c.is_active } }); void refresh(); }}>
                            {c.is_active ? "Aktywna" : "Wył."}
                          </Badge>
                          <Button size="icon" variant="ghost" onClick={() => setStatsFor(c)}><BarChart3 className="h-4 w-4" /></Button>
                          ...
                          <Button size="icon" variant="ghost"
                            onClick={async () => {
                              ...
                              toast.success("Usunięto"); void refresh();
                            }}>
      ...
      <Dialog open={!!statsFor} onOpenChange={(o) => !o && setStatsFor(null)}>
        <DialogContent className="max-w-3xl">
          <DialogHeader><DialogTitle>Statystyki: {statsFor?.name}</DialogTitle></DialogHeader>
...
function NewCampaignForm({ onDone }: { onDone: () => void }) {
  ...
  const [form, setForm] = useState({
    name: "", target_url: "", utm_source: "facebook", utm_medium: "cpc",
    utm_campaign: "", utm_term: "", utm_content: "", cost: 0, notes: "",
  });
  ...
  return (
    <div className="space-y-3">
      <div>
        ...
        <Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="np. FB Wiosna 2026" />
      </div>
      <div>
        <Label>URL docelowy *</Label>
        <Input value={form.target_url} onChange={(e) => setForm({ ...form, target_url: e.target.value })} placeholder="https://app.financeyou.pl/wniosek" />
      </div>
      <div className="grid grid-cols-2 gap-2">
        ...
        <div>
          ...
          <Input value={form.utm_campaign} onChange={(e) => setForm({ ...form, utm_campaign: e.target.value })} placeholder="auto ze slug" />
        </div>
        <div>
      ...
      <Button onClick={submit} disabled={saving || !form.name || !form.target_url} className="w-full">
...
function CampaignStats({ id }: { id: string }) {
  ...
  useEffect(() => { stats({ data: { id } }).then(setData).catch((e) => toast.error(e.message)); }, [id]);
  if (!data) return <div className="grid place-items-center py-8"><Loader2 className="h-5 w-5 animate-spin" /></div>;
  return (
      ...
      <div className="h-64">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data.series}>
            ...
            <Line type="monotone" dataKey="leads" stroke="hsl(var(--destructive))" strokeWidth={2} dot={false} />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
