...
function videoUrl(v: any): string | null {
...
function SzkoleniaInwestor() {
  ...
  useEffect(() => {
    void supabase.from("training_videos").select("*").eq("is_published", true)
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {videos.map((v) => {
          ...
          return (
            <Card key={v.id}>
              <CardHeader><CardTitle className="text-lg">{v.title}</CardTitle></CardHeader>
