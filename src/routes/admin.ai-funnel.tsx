...
function FunnelAnalyzerPage() {
  ...
  useEffect(() => {
    supabase.from("ai_landings").select("id, slug, title").order("created_at", { ascending: false })
  ...
  const load = async () => {
    ...
    try {
      ...
      setStats(r);
      const { data: ins } = await supabase.from("ai_funnel_insights")
        .select("*")
  ...
  useEffect(() => { load(); /* eslint-disable-next-line */ }, [days, landingId]);
          ...
          <div className="space-y-1">
            ...
            <Select value={landingId} onValueChange={setLandingId}>
              ...
              <SelectContent>
                ...
                {landings.map((l) => (<SelectItem key={l.id} value={l.id}>{l.title || l.slug}</SelectItem>))}
              </SelectContent>
            </Select>
          </div>
          <Button variant="outline" onClick={load} disabled={loading}>
            {loading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Filter className="mr-2 h-4 w-4" />} Odśwież
          </Button>
          <Button onClick={runAI} disabled={genLoading || !stats}>
            {genLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Sparkles className="mr-2 h-4 w-4" />} Analiza AI
          </Button>
        </CardContent>
      </Card>
      ...
      {stats && (
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <StatCard label="Przychód" value={`${stats.totals.revenue.toLocaleString("pl-PL")} zł`} />
          </div>

          <Card>
            <CardHeader>
              <CardTitle>Etapy lejka</CardTitle>
              <CardDescription>Unique sessions na każdym kroku z drop-off vs poprzedni etap.</CardDescription>
            </CardHeader>
            <CardContent className="p-0">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead>#</TableHead><TableHead>Etap</TableHead>
                    <TableHead>Sesje</TableHead><TableHead>Konwersja</TableHead><TableHead>Drop-off</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {stats.steps.map((s: any) => (
                    <TableRow key={`${s.order}-${s.step}`}>
                      ...
                      <TableCell><Badge variant="secondary">{s.conversion_rate}%</Badge></TableCell>
                      <TableCell>{s.drop_off_rate > 0 ? <span className="text-red-600">-{s.drop_off_rate}%</span> : "—"}</TableCell>
                    </TableRow>
                  ))}
                  {!stats.steps.length && <TableRow><TableCell colSpan={5} className="text-center text-muted-foreground py-6">Brak zdarzeń w tym okresie.</TableCell></TableRow>}
                </TableBody>
              </Table>
            </CardContent>
          </Card>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <Card>
              <CardHeader><CardTitle>Atrybucja źródeł</CardTitle><CardDescription>First-touch per sesja.</CardDescription></CardHeader>
              <CardContent className="p-0">
                <Table>
                  <TableHeader><TableRow>
                    <TableHead>Źródło / Medium</TableHead><TableHead>Sesje</TableHead><TableHead>Konw.</TableHead><TableHead>CR</TableHead>
                  ...
                  <TableBody>
                    ...
                    {!stats.sources.length && <TableRow><TableCell colSpan={4} className="text-center text-muted-foreground py-4">Brak danych.</TableCell></TableRow>}
                  </TableBody>
                </Table>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader><CardTitle>Urządzenia</CardTitle></CardHeader>
              <CardContent className="p-0">
                <Table>
                  <TableHeader><TableRow><TableHead>Urządzenie</TableHead><TableHead>Sesje</TableHead></TableRow></TableHeader>
                  <TableBody>
                    {stats.devices.map((d: any) => (
                      <TableRow key={d.device}><TableCell className="capitalize">{d.device}</TableCell><TableCell>{d.sessions}</TableCell></TableRow>
                    ))}
                    {!stats.devices.length && <TableRow><TableCell colSpan={2} className="text-center text-muted-foreground py-4">Brak danych.</TableCell></TableRow>}
                  </TableBody>
                </Table>
              </CardContent>
            </Card>
                    ...
                    <ul className="text-sm space-y-1">
                      {ins.bottlenecks.map((b: any, i: number) => (
                        <li key={i} className="flex gap-2">
                          <Badge variant="destructive" className="shrink-0">{b.drop_off_pct ?? "—"}%</Badge>
                          <span><strong>{b.step}:</strong> {b.diagnosis}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
                  ...
                  <div>
                    ...
                    <ul className="text-sm space-y-1">
                      {ins.recommendations.map((r: any, i: number) => (
                        <li key={i}>
                          <Badge variant="secondary" className="mr-2">{r.expected_impact ?? "?"}</Badge>
