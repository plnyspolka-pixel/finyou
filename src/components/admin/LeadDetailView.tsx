...
export function LeadDetailView({ id, compact = false, hideAdvancedTabs = false }: { id: string; compact?: boolean; hideAdvancedTabs?: boolean }) {
  ...
  if (q.isLoading) return <div className="p-6 text-muted-foreground text-sm">Ładowanie szczegółów…</div>;
  if (q.error) return <div className="p-6 text-destructive text-sm">Błąd: {(q.error as Error).message}</div>;
...
function QualitySection({ lead }: { lead: any }) {
  ...
  const mRescore = useMutation({
    ...
    onSuccess: (r: any) => { toast.success(`Przeliczono: Tier ${r.tier} (${r.reason})${r.capi?.ok ? " • event wysłany do Mety" : r.capi?.error ? ` • CAPI: ${r.capi.error}` : ""}`); invalidate(); },
    onError: (e: any) => toast.error(e.message),
  });
  const mGood = useMutation({
    mutationFn: () => goodFn({ data: { leadId: lead.id } }),
    onSuccess: () => { toast.success("Oznaczono jako TOP lead"); invalidate(); },
    onError: (e: any) => toast.error(e.message),
  });
  const mBad = useMutation({
    mutationFn: () => badFn({ data: { leadId: lead.id, reason } }),
    onSuccess: () => { toast.success("Oznaczono jako zły lead"); setBadOpen(false); setReason(""); invalidate(); },
    onError: (e: any) => toast.error(e.message),
  });
    ...
    <Card className="p-4 space-y-3">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex gap-2 flex-wrap">
          <Button size="sm" variant="outline" onClick={() => mRescore.mutate()} disabled={mRescore.isPending}><RefreshCw className="h-4 w-4 mr-1" /> Przelicz</Button>
          <Button size="sm" className="bg-green-600 hover:bg-green-700" onClick={() => mGood.mutate()} disabled={mGood.isPending}><ThumbsUp className="h-4 w-4 mr-1" /> Dobry lead</Button>
          {lead.marked_bad_lead ? (
            <Button size="sm" variant="outline" onClick={() => mUnbad.mutate()} disabled={mUnbad.isPending}>Cofnij spam</Button>
          ) : (
            <Button size="sm" variant="destructive" onClick={() => setBadOpen(!badOpen)}><ThumbsDown className="h-4 w-4 mr-1" /> Odrzuć / spam</Button>
          )}
        </div>
      </div>
      {badOpen && (
        <div className="flex gap-2 items-end pt-2 border-t">
          ...
          <Button size="sm" variant="destructive" onClick={() => mBad.mutate()} disabled={!reason.trim() || mBad.isPending}>Potwierdź</Button>
        </div>
      )}
      {lead.marked_bad_lead && <div className="text-xs text-destructive">⚠ Oznaczony jako zły lead: {lead.marked_bad_reason}</div>}
    </Card>
  );
}
    ...
    <div className="space-y-3">
      ...
      <Card className="p-0 overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {sends.length === 0 && <tr><td colSpan={7} className="p-4 text-center text-muted-foreground">Jeszcze nie wysłano żadnego maila z sekwencji.</td></tr>}
            {sends.map((s: any) => (
