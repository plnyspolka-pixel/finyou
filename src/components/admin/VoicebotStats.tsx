...
export function VoicebotStats({ rows }: { rows: any[] }) {
  ...
  function calc(set: any[]) {
    ...
    const avgDur = withDur.length ? Math.round(withDur.reduce((a, r) => a + (r.duration_seconds ?? 0), 0) / withDur.length) : null;
    const cost = set.reduce((a, r) => a + (typeof r.cost_credits === "number" ? r.cost_credits : 0), 0);
  ...
  return (
    <div className="grid gap-3 md:grid-cols-4">
      ...
      <Stat icon={<PhoneCall className="h-4 w-4" />} label="% odebranych" today={`${t.pctAnswered}%`} week={`${w.pctAnswered}%`} />
      <Stat icon={<Clock className="h-4 w-4" />} label="Średni czas" today={fmtDur(t.avgDur)} week={fmtDur(w.avgDur)} />
      <Stat icon={<Coins className="h-4 w-4" />} label="Koszt łącznie" today={fmtCost(t.cost)} week={fmtCost(w.cost)} />
    </div>
  );
}

function Stat({ icon, label, today, week }: { icon: React.ReactNode; label: string; today: React.ReactNode; week: React.ReactNode }) {
  return (
    <Card className="p-3">
