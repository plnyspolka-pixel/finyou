...
export function EditableField({ label, value, table, rowId, column, type = "text", options, display, onSaved }: Props) {
    ...
    <div className="flex items-center gap-2">
      ...
      {type === "select" && options ? (
        ...
        <Select value={draft === "true" ? "true" : "false"} onValueChange={setDraft}>
          ...
          <SelectContent><SelectItem value="true">Tak</SelectItem><SelectItem value="false">Nie</SelectItem></SelectContent>
        </Select>
      ) : (
        <Input
          autoFocus
      ...
      <Button size="icon" variant="ghost" className="h-8 w-8" disabled={saving} onClick={() => void save()}>
