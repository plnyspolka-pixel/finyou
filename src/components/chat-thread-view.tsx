...
export function ChatThreadView({ threadId, senderRole, title }: Props) {
  ...
  return (
    <Card>
      <CardHeader><CardTitle className="text-base">{title ?? "Rozmowa"}</CardTitle></CardHeader>
      <CardContent className="space-y-3">
        <div className="space-y-2 max-h-96 overflow-y-auto p-2 bg-muted/30 rounded-md">
          {messages.map((m) => {
            ...
            return (
              <div key={m.id} className={`flex ${mine ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[80%] rounded-md px-3 py-2 text-sm ${mine ? "bg-primary text-primary-foreground" : "bg-card border"}`}>
                  <div>{m.body_filtered ?? m.body}</div>
                  {Array.isArray(m.moderation_flags) && m.moderation_flags.length > 0 && (
                    <div className="mt-1 flex gap-1">
                      {m.moderation_flags.map((f: string) => <Badge key={f} variant="destructive" className="text-[10px]">AI: {f}</Badge>)}
                    </div>
                  )}
                </div>
              </div>
          ...
          {messages.length === 0 && <div className="text-center text-sm text-muted-foreground py-6">Brak wiadomości. Napisz pierwszą.</div>}
        </div>
        <div className="flex gap-2">
          <Textarea value={body} onChange={(e)=>setBody(e.target.value)} placeholder="Napisz wiadomość…" rows={2}/>
          <Button onClick={handleSend} disabled={sending}>Wyślij</Button>
        </div>
        <p className="text-[11px] text-muted-foreground">
          AI moderuje wiadomości — numery, e-maile, linki i nazwy komunikatorów są ukrywane. Cała komunikacja odbywa się w platformie.
        </p>
      </CardContent>
    </Card>
  );
