...
export function MetaRateButtons({
  ...
  const good = useMutation({
    ...
    onSuccess: () => { toast.success("Wysłano sygnał TOP do Meta"); onChanged?.(); },
  ...
  const bad = useMutation({
    ...
    onSuccess: () => { toast.success("Wysłano sygnał złego leada do Meta"); setReasonOpen(false); setReason(""); onChanged?.(); },
    onError: (e: any) => toast.error(e?.message ?? "Nie udało się"),
  });
  const unmark = useMutation({
    mutationFn: () => unmarkFn({ data: { leadId } }),
    onSuccess: () => { toast.success("Cofnięto oznaczenie"); onChanged?.(); },
    onError: (e: any) => toast.error(e?.message ?? "Nie udało się"),
  });
  ...
  if (markedBad) {
    return (
      <Button
        ...
        onClick={(e) => { e.preventDefault(); e.stopPropagation(); unmark.mutate(); }}
        disabled={busy}
      >
        {unmark.isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : (
          <><Undo2 className="h-3.5 w-3.5 mr-1" />Cofnij „zły"</>
        )}
      </Button>
    );
  }
  ...
  return (
    <>
      <div className="inline-flex items-center gap-1 rounded-md border border-white/20 bg-white/[0.06] p-0.5" title="Oceń leada dla Meta (Conversion API)">
        <Button
          size="sm"
          ...
          onClick={(e) => { e.preventDefault(); e.stopPropagation(); good.mutate(); }}
          disabled={busy}
          aria-label="Dobry lead — wyślij sygnał do Meta"
        >
          {good.isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : (
            <><ThumbsUp className="h-3.5 w-3.5 mr-1" />TOP</>
          )}
        </Button>
        <Button
          size="sm"
          className={`${sz} bg-rose-500/25 text-rose-50 border border-rose-300/40 hover:bg-rose-500/45`}
          onClick={(e) => { e.preventDefault(); e.stopPropagation(); setReasonOpen(true); }}
          disabled={busy}
          aria-label="Zły lead — wyślij sygnał do Meta"
        >
          <ThumbsDown className="h-3.5 w-3.5 mr-1" />Spam
        </Button>
      </div>
      ...
      <Dialog open={reasonOpen} onOpenChange={(v) => !v && !bad.isPending && setReasonOpen(false)}>
        <DialogContent>
          ...
          <p className="text-sm text-muted-foreground">
            Wyślemy do Meta negatywny sygnał (BadLead), żeby algorytm uczył się nie kierować podobnych leadów. Krótko opisz powód.
          </p>
          <Textarea
            value={reason}
          ...
          />
          <DialogFooter>
            <Button variant="ghost" onClick={() => setReasonOpen(false)} disabled={bad.isPending}>Anuluj</Button>
            <Button
              className="bg-rose-600 text-white hover:bg-rose-700"
              onClick={() => bad.mutate()}
              disabled={bad.isPending || reason.trim().length < 3}
            >
              {bad.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : "Wyślij sygnał do Meta"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
