...
import { analyzeLandingPerformance, applyOptimization, rejectOptimization } from "@/lib/ai-optimizer.functions";

export function MicroOptimizerDialog({
  landing, open, onClose,
}: { landing: { id: string; title: string }; open: boolean; onClose: () => void }) {
  ...
  async function load() {
    const { data } = await supabase
      ...
      .select("*").eq("landing_id", landing.id)
      .order("created_at", { ascending: false }).limit(50);
    setRows(data ?? []);
  }

  useEffect(() => { if (open) load(); }, [open, landing.id]);
  ...
  async function onApply(id: string) {
    setBusyId(id);
    try { await apply({ data: { id } }); toast.success("Zastosowano"); await load(); }
    catch (e: any) { toast.error(e.message); }
  ...
  async function onReject(id: string) {
    setBusyId(id);
    try { await reject({ data: { id } }); toast.success("Odrzucono"); await load(); }
    catch (e: any) { toast.error(e.message); }
    finally { setBusyId(null); }
  }
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2"><Wand2 className="h-5 w-5" /> AI Micro-Optimizer</DialogTitle>
          <DialogDescription>{landing.title}</DialogDescription>
        </DialogHeader>

        <div className="flex items-center justify-between gap-2">
          <p className="text-sm text-muted-foreground">
            AI analizuje wyświetlenia, scroll, kliknięcia i warianty, a potem proponuje konkretne zmiany.
          </p>
        ...
        <section className="space-y-2">
          ...
          {pending.length === 0 && <p className="text-sm text-muted-foreground">Brak nowych rekomendacji. Uruchom analizę.</p>}
          {pending.map((r) => (
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {r.expected_lift_pct != null && (
                      <Badge variant="secondary">+{Number(r.expected_lift_pct).toFixed(1)}% CR</Badge>
                    )}
                  ...
                  {r.error_message && <p className="text-xs text-destructive mt-1">Błąd: {r.error_message}</p>}
                </div>
                <div className="flex flex-col gap-1">
                  <Button size="sm" onClick={() => onApply(r.id)} disabled={busyId === r.id}>
                    <Check className="h-3.5 w-3.5 mr-1" /> Zastosuj
                  </Button>
                  <Button size="sm" variant="ghost" onClick={() => onReject(r.id)} disabled={busyId === r.id}>
        ...
        {history.length > 0 && (
          <section className="space-y-2">
            ...
            {history.map((r) => (
              <div key={r.id} className="rounded-md border p-2 text-xs flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Badge variant={r.status === "applied" ? "default" : r.status === "rejected" ? "destructive" : "secondary"}>
                    {r.status}
                  </Badge>
                  <span>{r.title}</span>
                </div>
