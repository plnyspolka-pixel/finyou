...
function Box({ title, children, done }: { title: string; children: React.ReactNode; done?: boolean }) {
  return (
    <Card className="border-amber-200 dark:border-amber-900/50">
      <CardHeader className="pb-3">
        <CardTitle className="flex items-center gap-2 text-sm font-semibold">
          {done ? <CheckCircle2 className="h-4 w-4 text-emerald-600" /> : <Circle className="h-4 w-4 text-amber-500" />}
          {title}
        </CardTitle>
      </CardHeader>
...
export function InlineMissingActions({ progress, userId, client, loan, property, onChanged }: Props) {
  ...
  if (!f.hasPropertyType && loan) {
    boxes.push(<PropertyTypeBox key="ptype" loanId={loan.id} property={property} onChanged={onChanged} />);
  }
  ...
  for (const m of progress.missing) {
    if (m.kind === "kw_number" && property) {
      boxes.push(<KwNumberBox key="kw" propertyId={property.id!} initial={property.land_register_number ?? ""} onChanged={onChanged} />);
    } else if (m.kind === "usable_area" && property) {
      boxes.push(<UsableAreaBox key="area" propertyId={property.id!} initial={property.area_sqm ?? null} onChanged={onChanged} />);
    } else if (loan) {
      boxes.push(
        <DocumentUploadBox
          key={`doc-${m.kind}`}
...
function ContactBox({ userId, client, onChanged }: { userId: string; client: ClientRow | null; onChanged: () => void }) {
  ...
  return (
    <Box title="Twoje dane kontaktowe">
      <div className="grid gap-2 sm:grid-cols-2">
        ...
        <div><Label className="text-xs">Telefon</Label><Input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} /></div>
        <div><Label className="text-xs">E-mail</Label><Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} /></div>
      </div>
      <Button size="sm" onClick={save} disabled={busy}>{busy ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Zapisuję…</> : "Zapisz dane"}</Button>
    </Box>
  );
}

function PropertyTypeBox({ loanId, property, onChanged }: { loanId: string; property: PropertyRow | null; onChanged: () => void }) {
  const [value, setValue] = useState<SecurityType | null>((property?.property_type as SecurityType | null) ?? null);
  const [busy, setBusy] = useState(false);
  const save = async () => {
    if (!value) { toast.error("Wybierz typ"); return; }
    setBusy(true);
    try {
      const res = property?.id
        ? await supabase.from("properties").update({ property_type: value }).eq("id", property.id)
        : await supabase.from("properties").insert({ loan_application_id: loanId, property_type: value });
      if (res.error) throw res.error;
  ...
  return (
    <Box title="Typ zabezpieczenia">
      ...
      <Button size="sm" onClick={save} disabled={busy || !value}>{busy ? "Zapisuję…" : "Zapisz wybór"}</Button>
    </Box>
  );
}
...
function LoanTermsBox({ loan, onChanged }: { loan: LoanRow; onChanged: () => void }) {
  ...
  const save = async () => {
    setBusy(true);
    try {
      const res = await supabase.from("loan_applications").update({
        loan_amount: amount, preferred_period_months: months, max_monthly_payment: maxPayment, annual_investor_rate: rate,
      }).eq("id", loan.id);
      if (res.error) throw res.error;
    ...
    <Box title="Warunki pożyczki">
      <div className="space-y-3">
        ...
        <div>
          ...
          <Slider value={[months]} min={3} max={72} step={1} onValueChange={(v) => setMonths(v[0] ?? months)} />
        </div>
        <div>
          <Label className="text-xs">Jaką ratę miesięczną możesz spłacać?</Label>
          <Input type="number" value={maxPayment} onChange={(e) => setMaxPayment(Number(e.target.value) || 0)} />
        </div>
        <div>
          <Label className="text-xs">Jakie wynagrodzenie inwestora proponujesz w skali roku? (%)</Label>
          <Input type="number" step="0.5" value={rate} onChange={(e) => setRate(Number(e.target.value) || 0)} />
        </div>
      </div>
      <Button size="sm" onClick={save} disabled={busy}>{busy ? "Zapisuję…" : "Zapisz warunki"}</Button>
    </Box>
  );
}
...
function InvestorDescBox({ loan, onChanged }: { loan: LoanRow; onChanged: () => void }) {
  ...
  const [busy, setBusy] = useState(false);
  const save = async () => {
    if (text.trim().length < 20) { toast.error("Dodaj kilka zdań — min. 20 znaków"); return; }
    setBusy(true);
    try {
      const res = await supabase.from("loan_applications").update({ investor_description: text.trim() }).eq("id", loan.id);
      if (res.error) throw res.error;
  ...
  return (
    <Box title="Krótki opis dla inwestora">
      <Textarea rows={4} value={text} onChange={(e) => setText(e.target.value)} placeholder="Dlaczego potrzebujesz finansowania, na co pójdą pieniądze, jak planujesz spłacić…" />
      <Button size="sm" onClick={save} disabled={busy}>{busy ? "Zapisuję…" : "Zapisz opis"}</Button>
    </Box>
  );
}

function KwNumberBox({ propertyId, initial, onChanged }: { propertyId: string; initial: string; onChanged: () => void }) {
  ...
  const save = async () => {
    ...
    setBusy(true);
    try {
      const res = await supabase.from("properties").update({ land_register_number: kw.trim().toUpperCase() }).eq("id", propertyId);
      if (res.error) throw res.error;
  ...
  return (
    <Box title="Numer księgi wieczystej">
      <Input value={kw} onChange={(e) => setKw(e.target.value.toUpperCase())} placeholder="np. WA1M/00123456/7" className="font-mono" />
      <p className="text-xs text-muted-foreground">Adres i dane nieruchomości pobierzemy automatycznie z KW.</p>
      <Button size="sm" onClick={save} disabled={busy || !kw.trim()}>{busy ? "Zapisuję…" : "Zapisz numer"}</Button>
    </Box>
  );
}

function UsableAreaBox({ propertyId, initial, onChanged }: { propertyId: string; initial: number | null; onChanged: () => void }) {
  ...
  return (
    <Box title="Powierzchnia użytkowa">
      <div className="flex items-center gap-2">
        <Input type="number" value={area} onChange={(e) => setArea(Number(e.target.value) || 0)} className="max-w-32" />
        <span className="text-sm text-muted-foreground">m²</span>
      </div>
      <Button size="sm" onClick={save} disabled={busy || area <= 0}>{busy ? "Zapisuję…" : "Zapisz"}</Button>
    </Box>
  );
}

function DocumentUploadBox({ userId, loanId, kind, label, onChanged }: { userId: string; loanId: string; kind: string; label: string; onChanged: () => void }) {
  const [busy, setBusy] = useState(false);
  const handle = async (file: File | undefined | null) => {
    if (!file) return;
    if (file.size > 20 * 1024 * 1024) { toast.error("Plik za duży (max 20 MB)"); return; }
    setBusy(true);
    ...
    try {
      ...
      onChanged();
    } catch (e: any) { toast.error(e?.message ?? "Błąd uploadu", { id: t }); }
    finally { setBusy(false); }
  };
  ...
  return (
    <Box title={label}>
      ...
      <label className="inline-flex">
        <input type="file" accept={accept} className="hidden" onChange={(e) => { const f = e.target.files?.[0]; e.target.value = ""; void handle(f); }} />
        <Button asChild size="sm" variant="cta" disabled={busy}>
          <span>{busy ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Wysyłam…</> : <><Upload className="mr-2 h-4 w-4" />Wybierz plik</>}</span>
        </Button>
      </label>
    </Box>
  );
