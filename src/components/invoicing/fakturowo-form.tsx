...
import { createFakturowoDocument } from "@/lib/fakturowo.functions";
...
export const SELLER_KEY = "fakturowo_seller_settings_v1";

export function loadSeller(): SellerSettings {
...
export function SellerForm({ seller, onSave }: { seller: SellerSettings; onSave: (s: SellerSettings) => void }) {
...
export type IssueFormDefaults = {
  /** Wstępnie wypełniona nazwa pozycji. */
  productName?: string;
  /** Wstępnie wypełnione uwagi na dokumencie. */
  notes?: string;
  /** Wstępnie wypełnione miejsce wystawienia. */
  place?: string;
  /** Wstępnie wypełniona wartość brutto. */
  gross?: number;
};
...
export function IssueForm({
  ...
  const [product, setProduct] = useState({
    name: defaults?.productName || SUGGESTED_PRODUCTS[0],
    quantity: 1,
    unit: "szt.",
    vatRate: seller.vatRate || "23",
    gross: defaults?.gross ?? 0,
  });
  const [extra, setExtra] = useState({
    place: defaults?.place || seller.place || "",
        ...
        <Card>
          ...
          <CardContent className="grid gap-4 md:grid-cols-2">
            ...
            <div className="md:col-span-2 space-y-1.5">
              ...
              <Textarea value={extra.notes} onChange={(e) => setExtra({ ...extra, notes: e.target.value })} rows={4} />
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="space-y-4">
        <Card>
          ...
          <CardContent className="space-y-3 text-sm">
            ...
            <div className="flex justify-between"><span className="text-muted-foreground">Brutto</span><span>{Number(product.gross).toFixed(2)} {seller.currency}</span></div>
...
export function DocumentList({ mineOnly = false }: { mineOnly?: boolean }) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      let query = supabase
        ...
        .limit(200);
      if (mineOnly && user?.id) query = query.eq("created_by", user.id);
