...
function TeaserGrid({ teaser }: { teaser: any }) {
  const rows: Array<[string, string | null]> = [
    [
      "Kwota",
      teaser?.loan_amount ? `${Number(teaser.loan_amount).toLocaleString("pl-PL")} zł` : null,
    ],
    ...
    [
      "Wartość szacunkowa",
      teaser?.estimated_value
        ? `${Number(teaser.estimated_value).toLocaleString("pl-PL")} zł`
        : null,
    ],
          ...
          {!kartaOpen ? (
            ...
            <>
              <div className="max-h-72 overflow-y-auto rounded-md border bg-muted/30 p-3 text-xs leading-relaxed">
                ...
                <p>
                  Data Dopasowania:{" "}
                  {karta?.dopasowanie?.data
                    ? new Date(karta.dopasowanie.data).toLocaleString("pl-PL")
                    : "—"}
                </p>
                <p className="mt-2 font-medium">Parametry Projektu</p>
                <p>
                  Kwota:{" "}
                  {karta?.parametry?.kwota_pln
                    ? `${Number(karta.parametry.kwota_pln).toLocaleString("pl-PL")} zł`
                    : "—"}{" "}
                  · okres: {karta?.parametry?.okres_miesiecy ?? "—"} mies. · oprocentowanie:{" "}
                  {karta?.parametry?.oprocentowanie_roczne ?? "—"}% · zabezpieczenie:{" "}
                  {karta?.parametry?.typ_zabezpieczenia ?? "—"} · lokalizacja:{" "}
                  {karta?.parametry?.lokalizacja ?? "—"}
                </p>
                <p className="mt-2">
                  Okres Ochronny: {karta?.okres_ochronny}. Kara Obejściowa: {karta?.kara_obejsciowa}{" "}
                  — za doprowadzenie do Transakcji Chronionej z pominięciem Mechanizmu
                  Zabezpieczenia Prowizji. Ujawnienie Identyfikujące nastąpi po akceptacji tej Karty
                  i zatwierdzeniu Karty Transferu Danych.
                </p>
...
function NdaAccessionCard({ accessions, onDone }: { accessions: any[]; onDone: () => void }) {
  ...
  return (
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="text-base">
          Przystąpienie spółki do NDA (Załącznik nr 1 NDA)
        </CardTitle>
        <p className="text-xs text-muted-foreground">
      ...
      <CardContent className="space-y-3">
        {accessions.map((a: any) => (
          ...
          >
            ...
            <Badge
              className={
                a.confirmed_at ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-800"
              }
            >
        ...
        {!open ? (
          ...
          <div className="space-y-3">
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div className="space-y-1">
                ...
                <Input
                  ...
                  onChange={(e) => setForm((f) => ({ ...f, registryNo: e.target.value }))}
                />
              </div>
              <div className="space-y-1">
            ...
            <Button
              size="sm"
              disabled={!confirmed || form.companyName.trim().length < 2 || mut.isPending}
              onClick={() => mut.mutate()}
            >
              {mut.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
              Zgłoś przystąpienie
