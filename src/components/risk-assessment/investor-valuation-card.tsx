  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Automatyczna prognoza dla tego tematu inwestycyjnego (dane rynkowe, bez danych osobowych klienta).</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4 text-sm">
        {/* Prognozowana wartość nieruchomości */}
        <div>
          <div className="text-xs text-muted-foreground mb-1">Prognozowana wartość nieruchomości</div>
        ...
        <div>
          <div className="text-xs text-muted-foreground mb-1 flex items-center gap-1"><Gavel className="h-3.5 w-3.5" /> Wartość szybkiej (wymuszonej) sprzedaży — licytacja</div>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-center">
            ...
            <div className="rounded-md border p-2">
              ...
              <div className="font-semibold">{fmtPln(qs.expectedLowPln)}–{fmtPln(qs.expectedHighPln)}</div>
            </div>
          </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-x-5 gap-y-1 text-xs">
            ...
            {sa.nearestLargeCity.name && (
              <span className="flex items-center gap-1"><MapPinned className="h-3.5 w-3.5 text-muted-foreground" /> {sa.nearestLargeCity.name}{sa.nearestLargeCity.distanceKm != null ? ` (${sa.nearestLargeCity.distanceKm} km)` : ""}</span>
            )}
            {sa.offersTotal > 0 && (
              <span className="flex items-center gap-1"><Building2 className="h-3.5 w-3.5 text-muted-foreground" /> Oferty w okolicy: <b>{sa.offersTotal}</b> (biura: {sa.offersAgency})</span>
            )}
            {sa.medianPricePerM2 != null && <span>Mediana ofert: <b>{fmtPln(sa.medianPricePerM2)}/m²</b></span>}
          </div>
          {drivers.length > 0 && (
