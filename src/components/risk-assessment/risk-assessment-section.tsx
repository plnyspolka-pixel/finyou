...
export function RiskAssessmentSection({ applicationId }: { applicationId: string }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {result && <RiskAssessmentReport result={result} />}
    </div>
  );
}

/**
 * Pełny raport oceny ryzyka inwestycji (bez akcji) — panel admina i szybka
 * analiza wniosku spoza Finance You w panelu inwestora.
 */
export function RiskAssessmentReport({ result }: { result: InvestmentRiskAssessment }) {
  return (
    <>
            ...
            <>
              ...
              <div className="flex flex-wrap gap-2 items-center">
                <Badge variant="outline">
                  Prawdopodobny wynik: {auctionOutcomeLabel(result.forcedSale.likelyAuctionOutcome)}
                </Badge>
      ...
      <Card>
        ...
        <CardContent className="text-sm space-y-1">
          ...
          <div>
            <span className="text-muted-foreground">Wiek / płeć:</span> {result.owner.age ?? "—"}{" "}
            ...
            <>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {result.kwLegal.owners.length > 1 && (
                  <Badge variant="outline">Współwłaściciele: {result.kwLegal.owners.length}</Badge>
                )}
              </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Tylko konkretne fakty, rozbieżności i sygnały ryzyka z treści — bez oceny zaangażowania
            czy sentymentu.
          </CardDescription>
        </CardHeader>
        <CardContent className="text-sm space-y-2">
