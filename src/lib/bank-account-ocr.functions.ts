...
export const verifyBankAccountDocument = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (!isPdf && !isImage) {
      return { ok: false, reason: "unsupported" as const, ibanMatch: false, holderMatch: false, foundIbans: [] as string[], foundHolder: "" };
    }

    const userContent: unknown[] = [
      { type: "text", text: "To jest dokument bankowy (potwierdzenie konta, wyciąg, umowa, blankiet przelewu). Wyodrębnij: 1) wszystkie numery rachunków bankowych (IBAN, 26 cyfr lub format PL + 26 cyfr), 2) pełne imię i nazwisko właściciela rachunku lub nazwę firmy (posiadacza konta). Zwróć WYŁĄCZNIE JSON w formacie: {\"ibans\":[\"...\"],\"holder\":\"...\"}. Bez komentarzy." },
    ];
    if (isImage) userContent.push({ type: "image_url", image_url: { url: data.dataUrl } });
    else userContent.push({ type: "file", file: { filename: data.fileName ?? "document.pdf", file_data: data.dataUrl } });
    ...
    try {
      const resp = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            { role: "system", content: "Jesteś asystentem OCR specjalizującym się w polskich dokumentach bankowych. Odpowiadasz wyłącznie poprawnym JSON-em." },
            { role: "user", content: userContent },
          ],
        }),
      });
      if (resp.status === 429) return { ok: false, reason: "rate_limited" as const, ibanMatch: false, holderMatch: false, foundIbans: [], foundHolder: "" };
      if (resp.status === 402) return { ok: false, reason: "ai_quota" as const, ibanMatch: false, holderMatch: false, foundIbans: [], foundHolder: "" };
      if (!resp.ok) return { ok: false, reason: "ai_error" as const, ibanMatch: false, holderMatch: false, foundIbans: [], foundHolder: "" };
      const json = await resp.json();
      text = json?.choices?.[0]?.message?.content ?? "";
    } catch {
      return { ok: false, reason: "ai_error" as const, ibanMatch: false, holderMatch: false, foundIbans: [], foundHolder: "" };
    }
    ...
    const ibanMatch = normalizedFound.some((i) => i.endsWith(expectedIban.slice(-20)) || i === expectedIban);

    const expectedTokens = normalizeName(data.expectedHolder).split(" ").filter((t) => t.length >= 3);
    const holderNorm = normalizeName(holder + " " + text);
    const matchedTokens = expectedTokens.filter((t) => holderNorm.includes(t));
    const holderMatch = expectedTokens.length > 0 && matchedTokens.length >= Math.min(2, expectedTokens.length);
