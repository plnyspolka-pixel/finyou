...
const COMPLETE: CompletenessInput = {
  loan_amount: 250000,
  client: { first_name: "Jan", last_name: "Kowalski", phone: "+48500600700", email: "jan@example.com" },
...
describe("evaluateApplicationCore", () => {
  ...
  it("wykrywa brak prawdziwego imienia i nazwiska (placeholder ze stuba leada)", () => {
    const r = evaluateApplicationCore({ ...COMPLETE, client: { first_name: "Lead", last_name: "—", phone: "500600700" } });
    expect(r.complete).toBe(false);
    expect(r.missing).toContain("name");
  });

  it("wymaga kontaktu — telefon LUB e-mail", () => {
    const noContact = evaluateApplicationCore({ ...COMPLETE, client: { first_name: "Jan", last_name: "Kowalski" } });
    expect(noContact.missing).toContain("contact");
    const emailOnly = evaluateApplicationCore({ ...COMPLETE, client: { first_name: "Jan", last_name: "Kowalski", email: "j@x.pl" } });
  ...
  it("wymaga kwoty pożyczki większej od zera", () => {
    ...
    expect(evaluateApplicationCore({ ...COMPLETE, loan_amount: "250000" }).missing).not.toContain("amount");
  });

  it("odrzuca śmieć w polu KW, ale akceptuje poprawny numer", () => {
    expect(evaluateApplicationCore({ ...COMPLETE, properties: [{ land_register_number: "PRZESŁANY", photos: ["a.jpg"] }] }).missing).toContain("kw");
    expect(evaluateApplicationCore({ ...COMPLETE, properties: [{ land_register_number: "WA1M/00123456/7", photos: ["a.jpg"] }] }).missing).not.toContain("kw");
  });

  it("akceptuje media jako zdjęcia ALBO dokumenty", () => {
    const docsOnly = evaluateApplicationCore({ ...COMPLETE, properties: [{ land_register_number: "WA1M/00123456/7", photos: [] }], docCount: 2 });
    expect(docsOnly.missing).not.toContain("media");
    const noMedia = evaluateApplicationCore({ ...COMPLETE, properties: [{ land_register_number: "WA1M/00123456/7", photos: [] }], docCount: 0 });
    expect(noMedia.missing).toContain("media");
  });

  it("zbiera wszystkie braki dla gołego stuba leada", () => {
    const r = evaluateApplicationCore({ loan_amount: null, client: { first_name: "Lead", last_name: "—" }, properties: [], docCount: 0 });
    expect(r.complete).toBe(false);
