/**
 * Walidacja danych wejściowych — DWIE warstwy:
 *   1. Schemat (patrz schema.ts / validateSchema)  — kształt, typy, wzorce
 *   2. Reguły biznesowe (waliduj_reguly)           — spójność, której schemat nie wyrazi
 *
 * Port z `validator.py`. Warstwa 2 jest ważniejsza: łapie dane, które
 * przechodzą walidację formalną, a mimo to dają wewnętrznie sprzeczną umowę.
 *
 * Kod wyjścia (patrz `kodWyjscia`): 0 = OK, 1 = błędy blokujące, 2 = ostrzeżenia.
 */

/* eslint-disable @typescript-eslint/no-explicit-any */
import { validateSchema } from "./schema";
import { walidujHarmonogram } from "./harmonogram";
import { Problem, type Poziom } from "./validator-types";
...
function naLiczbeBezp(txt: unknown): number | null {
  try {
    return naLiczbe(txt as string);
  } catch {
    return null;
  }
}

/** Formatowanie liczby jak Python `:,.2f` (przecinek tysięcy, kropka dziesiętna). */
function fmt(n: number): string {
  return n.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
}

function asList(x: any): Dane[] {
...
export function walidujReguly(d: Dane): Problem[] {
  const P: Problem[] = [];
  const blad = (sc: string, k: string) => P.push(new Problem("BLAD", sc, k));
  const ostrz = (sc: string, k: string) => P.push(new Problem("OSTRZEZENIE", sc, k));

  const nier: Dane[] = d.nieruchomosci ?? [];
  const por: Dane | null = d.porecziciel ?? null;
  const pbLista = asList(d.pozyczkobiorca);
  const zab: Dane = d.zabezpieczenia ?? {};

  // --- R1: unikalność ID nieruchomości ---
  ...
  nier.forEach((n, i) => {
    ...
    // --- R3: właściciel osoba trzecia musi mieć dane ---
    ...
    // --- R4: właściciel = poręczyciel, ale poręczyciela brak ---
    if (n.wlasciciel_ref === "porecziciel" && por === null) {
      blad(sc, "wlasciciel_ref='porecziciel', ale poręczyciel nie został zdefiniowany");
    }

    // --- R5: opróżnione miejsce wymaga wskazania wykreślanej hipoteki ---
    const hip: Dane = n.hipoteka ?? {};
    if (hip.pierwszenstwo === "oproznione_miejsce") {
      ...
      const usuwane = (n.obciazenia ?? []).filter((o: Dane) =>
    ...
    // --- R6: pierwsze miejsce przy istniejących hipotekach ---
    if (hip.pierwszenstwo === "pierwsze") {
      const pozostajace = (n.obciazenia ?? []).filter(
        (o: Dane) => o.dzial === "IV" && ["brak", "pozostaje_akceptowane"].includes(o.sposob_usuniecia ?? "brak"),
      );
    ...
    if (zakres === "po_odlaczeniu") {
      if (doOdlaczenia.length === 0) {
        ...
        if (dzialki.length && new Set(doOdlaczenia).size === new Set(dzialki).size && doOdlaczenia.every((x) => dzialki.includes(x))) {
          ostrz(sc, "Do odłączenia wskazano wszystkie działki z KW — odłączenie jest wtedy zbędne, wystarczy zakres='cala_kw'");
        }
      }
      ostrz(
        sc,
    ...
    (n.obciazenia ?? []).forEach((o: Dane, j: number) => {
      const osc = `${sc}.obciazenia[${j}]`;
      const sposob = o.sposob_usuniecia ?? "brak";

      // --- R7: zrzeczenie wymaga uprawnionego ---
      ...
      // --- R8: spłata ze środków wymaga kwoty i rachunku ---
      ...
      // --- R9: egzekucja administracyjna pozostawiona bez działania ---
      ...
      // --- R10: dożywocie/służebność bez usunięcia ---
      if (["dozywocie", "sluzebnosc_osobista"].includes(o.rodzaj) && ["brak", "pozostaje_akceptowane"].includes(sposob)) {
        ostrz(osc, "Służebność osobista/dożywocie pozostaje na nieruchomości — istotnie obniża wartość egzekucyjną zabezpieczenia");
      }
    });
  });

  // --- R11: suma spłat wierzycieli nie może przekroczyć kwoty pożyczki ---
  try {
    ...
    if (sumaSplat > kwotaPoz) {
      blad("warunki.kwota_pozyczki", `Suma spłat wierzycieli (${fmt(sumaSplat)}) przekracza Kwotę Pożyczki (${fmt(kwotaPoz)})`);
    } else if (sumaSplat > 0 && kwotaPoz - sumaSplat < 0.1 * kwotaPoz) {
      ostrz("warunki.kwota_pozyczki", `Po spłacie wierzycieli pożyczkobiorcy pozostaje ${fmt(kwotaPoz - sumaSplat)} zł (<10% kwoty pożyczki)`);
    }
  } catch {
    /* brak/niepoprawne dane — pomijamy, złapie warstwa schematu */
  }

  // --- R19: charakter hipoteki a liczba nieruchomości ---
  ...
  nier.forEach((n, i) => {
    if (n.zakres === "po_odlaczeniu") {
      const istniejace = (n.obciazenia ?? []).filter((o: Dane) => o.dzial === "IV");
  ...
  const podmioty: Array<[string, Dane, boolean]> = [];
  pbLista.forEach((o, i) => {
  ...
  const pd = d.pozyczkodawca;
  if (pd && pd.typ === "podmiot_gospodarczy") podmioty.push(["pozyczkodawca", pd, false]);
  for (const n of nier) {
  ...
  for (const [etykieta, o, jestDluznikiem] of podmioty) {
    ...
    // --- R23: spółka cywilna nie ma podmiotowości ---
    ...
    // --- R24: reprezentacja ---
    const repLista = asList(o.reprezentacja);
    ...
    // --- R27: uchwała zadeklarowana jako przedłożona, ale bez danych ---
    for (const pole of ["uchwala_zobowiazanie", "uchwala_nieruchomosc"] as const) {
      const u = o[pole];
      if (u && u.przedlozona && !(u.data || u.numer)) {
        ostrz(`${etykieta}.${pole}`, "Uchwała oznaczona jako przedłożona, ale bez daty i numeru — nie da się jej jednoznacznie zidentyfikować w umowie");
      }
    }
  }
  ...
  if (poddaje.includes("wlasciciel_osoba_trzecia")) {
    if (!nier.some((n) => n.wlasciciel_ref === "osoba_trzecia")) {
      blad("zabezpieczenia.egzekucja_777", "Wskazano właściciela-osobę trzecią, ale żadna nieruchomość nie ma takiego właściciela");
    }
  }
  ...
  const h: Dane = d.warunki?.harmonogram ?? {};
  ...
  const osoby: Array<[string, Dane | null]> = pbLista.map((o, i) => [
  ...
  for (const [etykieta, osoba] of osoby) {
    if (osoba && osoba.typ === "osoba_fizyczna") {
      if (osoba.stan_cywilny === "zonaty_zamezna" && !osoba.ustroj_majatkowy) {
        ostrz(etykieta, "Osoba w związku małżeńskim bez określonego ustroju majątkowego — nie da się ustalić, czy wymagana jest zgoda małżonka");
      }
    }
  }
...
/** Warstwa 1 + 2 + walidacja harmonogramu (jeśli tabela rat jest wypełniona). */
export function waliduj(dane: Dane): Problem[] {
  return [...validateSchema(dane), ...walidujReguly(dane), ...walidujHarmonogram(dane)];
}

export function kodWyjscia(problemy: Problem[]): 0 | 1 | 2 {
  const maBledy = problemy.some((p) => p.poziom === "BLAD");
  if (maBledy) return 1;
  return problemy.length ? 2 : 0;
}
