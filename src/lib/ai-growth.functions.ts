...
const sectionSchema = z.object({
  ...
  items: z.array(z.object({ title: z.string(), text: z.string().optional().default("") })).optional().default([]),
});
...
export const generateAiLanding = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const ctaUrl = data.ctaUrl ?? settings?.primary_cta_url ?? "https://app.financeyou.pl/embed/wniosek";
    ...
    const tools = [
      {
        ...
        function: {
          ...
          parameters: {
            type: "object",
            properties: {
              slug: { type: "string", description: "URL slug, małe litery, myślniki, max 60 znaków" },
              title: { type: "string" },
              ...
              sections: {
                ...
                items: {
                  type: "object",
                  properties: {
                    kind: { type: "string", enum: ["benefits", "how_it_works", "social_proof", "faq", "cta"] },
                    heading: { type: "string" },
                    body: { type: "string" },
                    items: {
                      type: "array",
            ...
            required: ["slug", "title", "meta_title", "meta_description", "hero_headline", "hero_subheadline", "cta_label", "sections"],
            additionalProperties: false,
          },
        },
      },
    ...
    while (true) {
      ...
      const { data: existing } = await supabase.from("ai_landings").select("id").eq("slug", candidate).maybeSingle();
    ...
    const { data: inserted, error } = await supabase.from("ai_landings").insert({
      slug,
      title: parsed.title,
      status,
      goal: data.goal,
...
export const deleteLanding = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (error) throw new Error(error.message);
    await supabase.from("ai_growth_action_log").insert({
      module: "landing_generator", action: "delete", status: "ok",
      summary: "Usunięcie landingu", payload: { landing_id: data.id }, actor: userId,
    });
    return { ok: true };
  });
...
export const updateGrowthSettings = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { supabase, userId } = context;
    const { data: row } = await supabase.from("ai_growth_settings").select("id").limit(1).maybeSingle();
    if (!row) throw new Error("Brak rekordu ustawień");
    const { error } = await supabase.from("ai_growth_settings").update({ ...data, updated_by: userId }).eq("id", row.id);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
