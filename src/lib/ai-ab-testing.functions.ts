...
export const createVariant = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: row, error } = await supabase.from("ai_landing_variants").insert({
    ...
    await supabase.from("ai_growth_action_log").insert({
      module: "ab_testing", action: "create_variant", status: "ok",
      summary: `Nowy wariant „${data.label}"`,
      payload: { landing_id: data.landing_id, variant_id: row.id }, actor: userId,
    });
    return { variant: row };
  });
...
export const generateVariantWithAI = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: landing, error: lErr } = await supabase.from("ai_landings").select("*").eq("id", data.landing_id).maybeSingle();
    ...
    const system = "Jesteś strategiem konwersji. Tworzysz alternatywny wariant landingu pod test A/B. " +
    ...
    await supabase.from("ai_growth_action_log").insert({
      module: "ab_testing", action: "ai_generate_variant", status: "ok",
      summary: `AI wygenerowało wariant „${data.label}"`,
      payload: { landing_id: data.landing_id, variant_id: row.id, angle: data.angle }, actor: userId,
    });
    return { variant: row };
  });
