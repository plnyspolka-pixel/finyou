...
import { borrowerTypeLabels, propertyTypeLabels, idDocumentTypeLabels } from "./client-profile-types";
import { borrowerDisplayName, formatDate, formatPLN, recommendSecurity } from "./client-profile-math";
...
export function generateApplicationDoc(p: ClientProfile): string {
  ...
  return [
    ...
    line("Maksymalna miesięczna rata akceptowana przez klienta", formatPLN(p.offerData.maxMonthlyPaymentByClient)),
...
export function generateContractDoc(p: ClientProfile, schedule: ScheduleData | null): string {
  ...
  return [
    ...
    line("Kwota nominalna pożyczki", formatPLN(schedule?.nominalLoanAmount ?? (o.netAmountToClient ?? 0) + (o.creditedCommission ?? 0))),
    ...
    line("Rachunek do spłat", inv.bankAccount),
    ``,
    `Prowizja jest kredytowana, powiększa nominalną kwotę pożyczki i jest spłacana wraz z kapitałem pożyczki. Harmonogram stanowi załącznik nr 1 do umowy.`,
    ...
    `8) wie, że prowizja jest kredytowana i powiększa nominalną kwotę pożyczki,`,
...
export function generateProtocolDoc(p: ClientProfile, schedule: ScheduleData | null): string {
  ...
  return [
    ...
    line("Data wypłaty", formatDate(o.payoutDate)),
    line("Sposób wypłaty środków", p.investorData.bankAccount ? `Przelew na rachunek ${p.investorData.bankAccount}` : "—"),
    ``,
    `..................................          ..................................`,
    `       Pożyczkodawca                                Pożyczkobiorca`,
  ].join("\n");
