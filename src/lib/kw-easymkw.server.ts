...
import { easyMkwJsonToExtraction, easyMkwStanZDnia } from "@/lib/kw-easymkw-map";
...
async function storeJson(kw: string, jobId: string, json: unknown) {
  // KLUCZOWE: oprócz surowego JSON-a zapisujemy też działy w formacie HTML
  // (identycznym jak z CMD). Bez tego analiza ryzyka i sprawdzenie
  // współwłaścicieli czytają puste kolumny dzial_* i „po cichu" zwracają
  // brak właścicieli/hipotek.
  const sections = renderKwSections(easyMkwJsonToExtraction(json));
  const stan = easyMkwStanZDnia(json);
  await supabaseAdmin
    .from("kw_documents")
    .update({
      status: "ready",
      ...
      okladka:
        `<table class="kw-ocr">` +
        `<tr><td class="csTytul">Numer księgi wieczystej</td><td class="csDane">${kw}</td></tr>` +
        (stan ? `<tr><td class="csTytul">Stan z dnia</td><td class="csDane">${stan}</td></tr>` : "") +
        `<tr><td class="csTytul">Źródło treści</td><td class="csDane">EasyMKW (Centralna Baza Ksiąg Wieczystych)</td></tr>` +
        `</table>`,
