...
export const addKeyword = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: row, error } = await supabase.from("ai_serp_keywords").insert({
      user_id: userId,
...
async function fetchSerpViaFirecrawl(keyword: string, language: string, location: string) {
  ...
  for (const link of links) {
    ...
    const host = (() => { try { return new URL(link).hostname.toLowerCase(); } catch { return ""; } })();
    if (!host) continue;
    if (/(^|\.)google\./.test(host)) continue;
    if (/(^|\.)gstatic\./.test(host) || /(^|\.)googleusercontent\./.test(host) || /(^|\.)youtube\.com$/.test(host) === false && /(^|\.)ytimg\./.test(host)) continue;
  ...
  if (m.includes("people also ask") || m.includes("podobne pytania")) features.push("people_also_ask");
  if (m.includes("featured snippet") || m.includes("polecany fragment")) features.push("featured_snippet");
...
export const checkKeywordRanking = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: kw, error: kErr } = await supabase
      .from("ai_serp_keywords").select("*").eq("id", data.keyword_id).single();
    ...
    const { data: prev } = await supabase
      .from("ai_serp_rankings").select("position")
      .eq("keyword_id", kw.id).order("checked_at", { ascending: false }).limit(1);
    const previous_position = prev?.[0]?.position ?? null;

    const { data: row, error: rErr } = await supabase.from("ai_serp_rankings").insert({
      keyword_id: kw.id,
      user_id: userId,
      position,
      previous_position,
...
export const checkAllRankings = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data: kws } = await supabase
      .from("ai_serp_keywords").select("id").eq("is_active", true);
    ...
    for (const k of kws) {
      try {
        ...
        const { data: kw } = await supabase.from("ai_serp_keywords").select("*").eq("id", k.id).single();
        if (!kw) continue;
        const { organic, features } = await fetchSerpViaFirecrawl(kw.keyword, kw.language, kw.location);
        let position: number | null = null;
        let foundUrl: string | null = null;
        if (kw.target_url) {
          ...
          for (let i = 0; i < organic.length; i++) {
            ...
            if (h === t || h.endsWith("." + t)) { position = i + 1; foundUrl = organic[i]; break; }
          }
        }
        const { data: prev } = await supabase
          .from("ai_serp_rankings").select("position")
        ...
        await supabase.from("ai_serp_rankings").insert({
          ...
          position, previous_position: prev?.[0]?.position ?? null,
          url: foundUrl, serp_features: features,
        });
        checked++;
        // gentle delay
