...
export const addOutreachTarget = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: row, error } = await supabase.from("ai_outreach_targets").insert({
      ...data,
...
export const updateOutreachTarget = createServerFn({ method: "POST" })
  ...
  .inputValidator((i) => z.object({
    ...
    status: z.enum(["new","queued","contacted","responded","won","rejected","blacklist"]).optional(),
    notes: z.string().max(2000).optional().nullable(),
  }).parse(i))
  .handler(async ({ data, context }) => {
    const { id, ...patch } = data;
...
export const discoverOutreachTargets = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { supabase, userId } = context;
    const { data: settings } = await supabase.from("ai_growth_settings").select("default_model, brand_name").limit(1).maybeSingle();
    const model = settings?.default_model ?? "google/gemini-2.5-flash";

    const system = "Jesteś specjalistą SEO/PR. Proponujesz realne polskie portale, blogi i serwisy " +
    ...
    const res = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        tools, tool_choice: { type: "function", function: { name: "propose_targets" } },
      }),
    });
    ...
    for (const t of items) {
      const domain = String(t.domain || "").trim().toLowerCase().replace(/^https?:\/\//, "").replace(/\/.*$/, "");
      if (!domain || !/^[a-z0-9.-]+\.[a-z]{2,}$/.test(domain)) { skipped.push(domain); continue; }
      ...
      if (!error) inserted++; else skipped.push(domain);
    }

    await supabase.from("ai_growth_action_log").insert({
      module: "outreach", action: "ai_discovery", status: "ok",
      summary: `AI zaproponowało ${items.length} portali (${inserted} dodanych)`,
      payload: { niche: data.niche, inserted, skipped }, actor: userId,
    });
...
export const generateOutreachMessage = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: target, error: tErr } = await supabase.from("ai_outreach_targets").select("*").eq("id", data.target_id).maybeSingle();
    if (tErr || !target) throw new Error("Target nie istnieje");

    const { data: settings } = await supabase.from("ai_growth_settings").select("default_model, brand_name, brand_description, primary_cta_url").limit(1).maybeSingle();
    const model = settings?.default_model ?? "google/gemini-2.5-flash";
    const brand = settings?.brand_name ?? "Finance You";

    let parentCtx = "";
    if (data.parent_id) {
      const { data: parent } = await supabase.from("ai_outreach_messages").select("subject, body, reply_excerpt").eq("id", data.parent_id).maybeSingle();
      if (parent) parentCtx = `\nPoprzedni mail:\nSubject: ${parent.subject}\n${parent.body}\n${parent.reply_excerpt ? `Odpowiedź odbiorcy: ${parent.reply_excerpt}` : "(brak odpowiedzi — to follow-up)"}`;
    }

    const stepName = data.step === 1 ? "pierwszy kontakt" : data.step === 2 ? "follow-up #1 (przypomnienie)" : `follow-up #${data.step - 1}`;

    const system = `Jesteś specjalistą outreach/PR dla marki ${brand} (${settings?.brand_description ?? ""}). ` +
    ...
    const userMsg = [
      ...
      settings?.primary_cta_url ? `Link do podlinkowania (jeśli stosowne): ${settings.primary_cta_url}` : "",
      parentCtx,
    ].filter(Boolean).join("\n");
    ...
    const res = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model,
        messages: [{ role: "system", content: system }, { role: "user", content: userMsg }],
        tools, tool_choice: { type: "function", function: { name: "write_outreach" } },
      }),
    });
    if (res.status === 429) throw new Error("Zbyt wiele zapytań do AI.");
    if (res.status === 402) throw new Error("Wyczerpany limit AI.");
    ...
    const { data: msg, error } = await supabase.from("ai_outreach_messages").insert({
    ...
    await supabase.from("ai_growth_action_log").insert({
      module: "outreach", action: "ai_generate_message", status: "ok",
      summary: `Wygenerowano ${stepName} dla ${target.domain}`,
      payload: { target_id: data.target_id, message_id: msg.id }, actor: userId,
    });
...
export const updateOutreachMessage = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: msg, error } = await supabase.from("ai_outreach_messages").update(patch).eq("id", data.id).select().single();
    if (error) throw new Error(error.message);

    if (data.mark_sent && msg) {
      await supabase.from("ai_outreach_targets").update({ status: "contacted" }).eq("id", msg.target_id).neq("status", "won").neq("status", "rejected");
      await supabase.from("ai_growth_action_log").insert({
        module: "outreach", action: "mark_sent", status: "ok",
        summary: `Mail wysłany (step ${msg.step})`,
        payload: { message_id: msg.id, target_id: msg.target_id }, actor: userId,
      });
    }
    return { ok: true };
  });
...
export const deleteOutreachMessage = createServerFn({ method: "POST" })
  ...
  .inputValidator((i) => z.object({ id: z.string().uuid() }).parse(i))
  .handler(async ({ data, context }) => {
    const { error } = await context.supabase.from("ai_outreach_messages").delete().eq("id", data.id);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
