...
export type KrsAddress = {
  country: string; voivodeship: string; county: string; municipality: string;
  city: string; postalCode: string; street: string; buildingNumber: string;
  apartmentNumber: string; fullAddress: string;
};

export type KrsPerson = { firstName: string; lastName: string; role?: string; pesel?: string };

export type KrsCompany = {
  name: string; krs: string; nip: string; regon: string;
  legalForm: string; status: string;
  registrationDate: string; deletionDate: string;
  address: KrsAddress;
  court: string;
  shareCapital: string;
  ...
  flags: {
    liquidation: boolean; bankruptcy: boolean; restructuring: boolean;
    suspended: boolean; deleted: boolean;
    section4HasEntries: boolean; section5HasEntries: boolean;
    section6HasEntries: boolean;
    representationMissing: boolean;
  };
  sections: {
    section1: any; section2: any; section3: any;
    section4: any; section5: any; section6: any;
  };
  raw: any;
};
...
export type KrsLookupResult =
  ...
  | { success: false; errorCode: "NOT_FOUND" | "INVALID_KRS" | "KRS_API_ERROR" | "KRS_TIMEOUT"; message: string };
...
function pickAddress(adr: any): KrsAddress {
  ...
  return {
    country, voivodeship: a.wojewodztwo ?? "", county: a.powiat ?? "",
    municipality: a.gmina ?? "", city, postalCode: postal,
    street, buildingNumber: building, apartmentNumber: apt, fullAddress: full,
  };
}
...
function mapKrs(raw: any, normalizedKrs: string): KrsCompany {
  ...
  const proxies: KrsPerson[] = (Array.isArray(prokurenciSrc) ? prokurenciSrc : []).map((p: any) => ({
    ...pickPersonOsoba(p),
    role: p?.rodzajProkury ?? "Prokurent",
  }));

  // Wspólnicy / udziałowcy (różne lokalizacje)
  const wspolnicySrc = d1?.wspolnicySpzoo
    ?? d1?.wspolnicy
    ?? d1?.akcjonariusze
    ?? [];
  ...
  const pkdMain = Array.isArray(pkdSrc) && pkdSrc[0]
    ? `${pkdSrc[0].kodPkd ?? ""} ${pkdSrc[0].opis ?? ""}`.trim()
    : "";
  const pkdAdditional: string[] = (Array.isArray(pkdAddSrc) ? pkdAddSrc : [])
    .map((p: any) => `${p.kodPkd ?? ""} ${p.opis ?? ""}`.trim());
  ...
  const restructuring = nonEmpty(d6?.postepowanieRestrukturyzacyjne) || nonEmpty(d6?.postepowanieNaprawcze);
  ...
  const section4HasEntries =
    nonEmpty(d4?.zaleglosci) || nonEmpty(d4?.wierzytelnosci) ||
    nonEmpty(d4?.zabezpieczenia) || Object.keys(d4 || {}).length > 0;
  const section5HasEntries = nonEmpty(d5?.kurator) || Object.keys(d5 || {}).length > 0;
  const section6HasEntries = Object.keys(d6 || {}).length > 0;

  return {
    name, krs: normalizedKrs, nip, regon, legalForm, status,
    registrationDate, deletionDate, address, court, shareCapital,
    representation: { method: repMethod, bodyName: repBody, members: repMembers },
    managementBoard, supervisoryBoard, proxies, partnersOrShareholders,
    pkd: { main: pkdMain, additional: pkdAdditional },
    flags: {
      liquidation, bankruptcy, restructuring, suspended, deleted,
      section4HasEntries, section5HasEntries, section6HasEntries,
      representationMissing: !repMethod && repMembers.length === 0 && managementBoard.length === 0,
    },
    sections: { section1: d1, section2: d2, section3: d3, section4: d4, section5: d5, section6: d6 },
    raw,
  };
}
...
async function fetchKrsOnce(krs: string, rejestr: "P" | "S"): Promise<{ status: number; body: any }> {
...
async function fetchKrs(krs: string): Promise<{ ok: true; body: any } | { ok: false; code: "NOT_FOUND" | "KRS_API_ERROR" | "KRS_TIMEOUT" }> {
  for (const rejestr of ["P", "S"] as const) {
    ...
    while (attempt < 2) {
      try {
        ...
        if (status >= 500) { attempt++; continue; }
  ...
  .handler(async ({ data }): Promise<KrsLookupResult> => {
    ...
    if (!result.ok) {
      await supabaseAdmin.from("external_api_logs").insert({
        provider: "KRS", query_type: "krs", query_value: normalized,
        success: false, error_code: result.code, response_time_ms: elapsed,
      });
      const msg =
        result.code === "NOT_FOUND" ? "Nie znaleziono podmiotu w KRS." :
        result.code === "KRS_TIMEOUT" ? "API KRS nie odpowiedziało w wymaganym czasie." :
    ...
    await supabaseAdmin.from("external_api_logs").insert({
      provider: "KRS", query_type: "krs", query_value: normalized,
      success: true, error_code: null, response_time_ms: elapsed,
    });
