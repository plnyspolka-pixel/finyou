...
const partners: Record<string, PartnerLike> = {
  p1: { id: "p1", sponsor_partner_id: "p2", status: "active", settlement_type: "b2b", bank_account_encrypted: "x" },
  p2: { id: "p2", sponsor_partner_id: "p3", status: "active", settlement_type: "unregistered_activity", bank_account_encrypted: "x" },
  p3: { id: "p3", sponsor_partner_id: null, status: "active", settlement_type: "b2b", bank_account_encrypted: "x" },
...
describe("getBasisAmount", () => {
  const ev = { gross_payment_amount: 1000, net_revenue_amount: 800, finance_you_fee_amount: 500, loan_amount: 200000 };
  it("fixed_amount → 1", () => expect(getBasisAmount(ev, "fixed_amount")).toBe(1));
  it("percent_of_gross_payment", () => expect(getBasisAmount(ev, "percent_of_gross_payment")).toBe(1000));
  it("percent_of_net_revenue", () => expect(getBasisAmount(ev, "percent_of_net_revenue")).toBe(800));
  it("percent_of_finance_you_fee", () => expect(getBasisAmount(ev, "percent_of_finance_you_fee")).toBe(500));
  it("percent_of_loan_amount", () => expect(getBasisAmount(ev, "percent_of_loan_amount")).toBe(200000));
...
describe("calculateAmount", () => {
  ...
  it("procent od bazy", () => {
    expect(calculateAmount({ basis_type: "percent_of_finance_you_fee", percent_rate: 20 }, 500)).toBe(100);
  });
  it("min_commission podnosi kwotę", () => {
    expect(calculateAmount({ basis_type: "percent_of_finance_you_fee", percent_rate: 1, min_commission: 50 }, 500)).toBe(50);
  });
  it("max_commission ogranicza kwotę", () => {
    expect(calculateAmount({ basis_type: "percent_of_finance_you_fee", percent_rate: 50, max_commission: 100 }, 500)).toBe(100);
  });
...
describe("decideCommissionStatus (forma rozliczenia + limit)", () => {
  it("B2B → requires_invoice", () => {
    const d = decideCommissionStatus({ settlementType: "b2b", grossAmount: 100, quarterApprovedSum: 0, limitAmount: 10000 });
  ...
  it("działalność nierejestrowana w limicie → pending_admin_approval + flaga oświadczenia", () => {
    const d = decideCommissionStatus({ settlementType: "unregistered_activity", grossAmount: 100, quarterApprovedSum: 0, limitAmount: 10813.5 });
  ...
  it("działalność nierejestrowana przekraczająca limit → requires_business_registration", () => {
    const d = decideCommissionStatus({ settlementType: "unregistered_activity", grossAmount: 1000, quarterApprovedSum: 10500, limitAmount: 10813.5 });
  ...
  it("brak skonfigurowanego limitu → wymaga rejestracji działalności (bezpiecznie)", () => {
    const d = decideCommissionStatus({ settlementType: "unregistered_activity", grossAmount: 100, quarterApprovedSum: 0, limitAmount: null });
    expect(d.requiresBusinessRegistration).toBe(true);
  });
});
...
describe("unregisteredLimitState", () => {
  it("oznacza zbliżanie się do limitu od 80%", () => {
    const s = unregisteredLimitState({ quarterApprovedSum: 8700, newAmount: 0, limitAmount: 10813.5 });
...
describe("wouldCreateCycle (zakaz pętli w strukturze)", () => {
  ...
  it("dopuszcza prawidłową zmianę", () => {
    const flat: Record<string, PartnerLike> = { a: { id: "a", sponsor_partner_id: null }, b: { id: "b", sponsor_partner_id: null } };
...
describe("groupPayouts (próg 500 zł + dane bankowe)", () => {
  ...
  it("pomija partnera bez rachunku bankowego", () => {
    const res = groupPayouts(
      [{ id: "c1", partner_id: "p4", gross_amount: 800 }],
      partners,
    );
    expect(res.items).toHaveLength(0);
