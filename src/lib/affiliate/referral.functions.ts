 ...
 */
export async function resolveReferralPartnerId(db: SupabaseClient, code: string): Promise<string | null> {
  const norm = String(code ?? "").trim();
  if (!norm) return null;

  const { data: ap } = await db.from("affiliate_partners").select("id").eq("referral_code", norm.toUpperCase()).maybeSingle();
  if (ap) return (ap as { id: string }).id;

  const { data: prof } = await db.from("profiles").select("user_id").eq("referral_code", norm).maybeSingle();
  const refUser = (prof as { user_id: string | null } | null)?.user_id;
  if (refUser) {
    const { data: ap2 } = await db.from("affiliate_partners").select("id").eq("user_id", refUser).maybeSingle();
...
export const captureReferral = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: self } = await affiliateDb.from("affiliate_partners").select("id").eq("user_id", context.userId).maybeSingle();
    if (self && (self as { id: string }).id === partnerId) return { captured: false, reason: "self" as const };

    // First-touch: ustaw tylko, gdy jeszcze puste.
    const { data: prof } = await affiliateDb.from("profiles").select("referred_by_partner_id").eq("user_id", context.userId).maybeSingle();
