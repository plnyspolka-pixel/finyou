...
export const addBacklink = createServerFn({ method: "POST" })
  ...
  .inputValidator((i) => z.object({
    ...
    link_type: z.enum(["editorial","guest_post","directory","forum","comment","partnership","other"]).default("editorial"),
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: row, error } = await supabase.from("ai_backlinks").insert({
    ...
    await supabase.from("ai_growth_action_log").insert({
      module: "link_building", action: "add_backlink", status: "ok",
      summary: `Dodano backlink z ${row.source_domain}`,
      payload: { backlink_id: row.id }, actor: userId,
    });
    return { backlink: row };
  });
...
export const generateLinkBuildingSuggestions = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [{ data: settings }, { data: existing }] = await Promise.all([
      supabase.from("ai_growth_settings").select("default_model, brand_name, brand_description").limit(1).maybeSingle(),
          ...
          properties: {
            suggestions: {
              ...
              items: {
                type: "object",
                properties: {
                  target_domain: { type: "string", description: "Domena bez https i www, np. example.pl" },
                  contact_hint: { type: "string", description: "Sugerowana ścieżka kontaktu (np. redakcja@..., formularz /kontakt)." },
                  strategy: { type: "string", description: "Krótka strategia (np. 'guest post o XYZ', 'link wymiany z artykułu o ABC')." },
                  rationale: { type: "string", description: "Dlaczego ta domena pasuje — w kontekście niszy." },
                  priority: { type: "integer", minimum: 1, maximum: 5 },
                  expected_da: { type: "integer", minimum: 0, maximum: 100, description: "Estymacja Domain Authority." },
                },
    ...
    const res = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        tools, tool_choice: { type: "function", function: { name: "propose_link_targets" } },
      }),
    });
    ...
    const { data: inserted, error: insErr } = await supabase
      .from("ai_linkbuilding_suggestions").insert(rows).select();
    if (insErr) throw new Error(insErr.message);

    await supabase.from("ai_growth_action_log").insert({
      module: "link_building", action: "generate_suggestions", status: "ok",
      summary: `AI zaproponowało ${rows.length} celów link-buildingu (nisza: ${data.niche})`,
      payload: { niche: data.niche }, actor: userId,
    });
...
export const updateLbSuggestion = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { id, ...patch } = data;
    const { error } = await context.supabase.from("ai_linkbuilding_suggestions").update(patch).eq("id", id);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
...
export const promoteSuggestionToOutreach = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: s, error } = await supabase
      .from("ai_linkbuilding_suggestions").select("*").eq("id", data.id).maybeSingle();
    ...
    await supabase.from("ai_linkbuilding_suggestions")
      .update({ status: "in_progress" }).eq("id", data.id);
