...
function moderate(body: string): { filtered: string; flags: string[]; blocked: boolean } {
  ...
  if (phoneRe.test(body)) { flags.push("phone"); filtered = filtered.replace(phoneRe, "[numer ukryty]"); }
  if (emailRe.test(body)) { flags.push("email"); filtered = filtered.replace(emailRe, "[e-mail ukryty]"); }
  if (urlRe.test(body)) { flags.push("url"); filtered = filtered.replace(urlRe, "[link ukryty]"); }
  if (messengerRe.test(body)) { flags.push("messenger"); filtered = filtered.replace(messengerRe, "[komunikator ukryty]"); }
...
export const sendChatMessage = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { error, data: row } = await context.supabase.from("chat_messages").insert({
    ...
    await context.supabase.from("chat_threads").update({ updated_at: new Date().toISOString() }).eq("id", data.threadId);
...
export const openOrCreateThread = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { data: existing } = await context.supabase
      ...
      .eq("investor_id", data.investorId).maybeSingle();
    if (existing) return { threadId: existing.id };
    const { data: created, error } = await context.supabase.from("chat_threads").insert({
