...
import { estimateForcedSale, auctionOutcomeLabel, residentialAuctionBlockRisk } from "./forced-sale";

describe("residentialAuctionBlockRisk — art. 952¹ § 2 KPC (próg 5%)", () => {
  it("mieszkanie: pożyczka < 5% wartości → blokada licytacji", () => {
    const r = residentialAuctionBlockRisk({ propertyType: "mieszkanie", loanAmountPln: 30_000, propertyValuePln: 800_000 });
  ...
  it("dom: pożyczka ≥ 5% → brak blokady", () => {
    const r = residentialAuctionBlockRisk({ propertyType: "dom", loanAmountPln: 60_000, propertyValuePln: 500_000 });
    expect(r.blocked).toBe(false);
  });
  it("dom: pożyczka < 5% → blokada", () => {
    expect(residentialAuctionBlockRisk({ propertyType: "dom", loanAmountPln: 20_000, propertyValuePln: 500_000 }).blocked).toBe(true);
  });
  it("grunt rolny: reguła nie dotyczy (nie mieszkaniowa)", () => {
    const r = residentialAuctionBlockRisk({ propertyType: "grunt_rolny", loanAmountPln: 1_000, propertyValuePln: 500_000 });
    expect(r.applicable).toBe(false);
    expect(r.blocked).toBe(false);
  });
  it("brak wartości → reguła nieokreślona", () => {
    expect(residentialAuctionBlockRisk({ propertyType: "mieszkanie", loanAmountPln: 10_000, propertyValuePln: null }).applicable).toBe(false);
  });
});
...
describe("estimateForcedSale — licytacje komornicze wg KPC", () => {
  ...
  it("wysoka łatwość sprzedaży → sprzedaż na I licytacji, bezpieczny bufor", () => {
    const r = estimateForcedSale({ basisValuePln: 1_000_000, basisSource: "test", saleabilityScore: 80, requestedLoanPln: 400_000 });
  ...
  it("niska łatwość sprzedaży → przejęcie wierzyciela, pożyczka > odzysk", () => {
    const r = estimateForcedSale({ basisValuePln: 1_000_000, basisSource: "test", saleabilityScore: 20, requestedLoanPln: 800_000 });
  ...
  it("umiarkowana łatwość → II licytacja", () => {
    const r = estimateForcedSale({ basisValuePln: 500_000, basisSource: "test", saleabilityScore: 45 });
