...
import { kwTools } from "./tools/kw";
import { contractTools } from "./tools/contracts";
import { clientDossierTools } from "./tools/client-dossier";
...
export default defineMcp({
  ...
  version: "0.9.0",
  instructions:
    "Narzędzia platformy Finance You (pożyczki pozabankowe + inwestycje). Obejmują CRM (leady, klienci, wnioski, oferty inwestorów), treści (blog, FAQ, landing pages, materiały marketingowe, szkolenia), program pośredników, windykację, czat klient↔inwestor oraz kalkulator raty. Dla zespołu (administrator/operator): `get_updates_since` — raport „co nowego” od wskazanego momentu (zacznij od niego, gdy użytkownik pyta o nowości; zapamiętaj `next_since` na kolejne pytanie), `list_inbox_threads` — skrzynka z wątkami czekającymi na odpowiedź, `read_inbox_thread` — pełna korespondencja z jedną osobą. Narzędzia działają w obie strony: poza odczytem są akcje panelu (edycja leadów, klientów, wniosków, decyzje o ofertach i propozycjach, kryteria instytucji, dostępy, windykacja, treści) oraz wysyłki (`send_email`, `send_sms`, `send_messenger_message`, `send_chat_reply`, `reply_institution_thread`). Boty i głos ElevenLabs: `eleven_status`, `eleven_list_agents`, `get_text_agent_prompt` / `update_text_agent_prompt`, `eleven_get_conversation` (transkrypty), `place_voice_call` (telefon Anią — realne połączenie), `ask_voice_agent` (tura testowa), `text_to_speech`, `speech_to_text`, `compose_music`, `create_dubbing`, `generate_video`; Twilio: `twilio_status`, `list_twilio_messages`, `list_twilio_calls`, `get_twilio_recording`, `twilio_place_call`; Meta (Facebook, Instagram, Messenger, reklamy, formularze leadów, piksel): `meta_status`, `list_facebook_posts`, `publish_facebook_post`, `list_instagram_media`, `publish_instagram_post`, `list_messenger_conversations`, `get_meta_campaigns_live`, `get_meta_ads_insights`, `update_meta_ad_status`, `get_meta_form_leads`; YouTube: `youtube_status`, `list_youtube_videos`, `list_youtube_comments`, `reply_youtube_comment`, `update_youtube_video`, `queue_youtube_publication`; HeyGen i Studio publikacji (filmy z awatarem Filipa): `heygen_status`, `generate_studio_script`, `create_studio_video_job`, `get_studio_job`, `poll_studio_jobs`, `publish_studio_job`, `generate_avatar_video`, `get_heygen_video`, `list_avatar_faqs`, `generate_avatar_faq_video`; Google (pozycjonowanie i ruch): `google_search_status`, `get_search_performance`, `get_search_trend`, `get_top_queries`, `get_top_pages`, `get_page_search_data`, `compare_search_periods`, `inspect_url`, `list_sitemaps`, `get_site_traffic` (GA4), `get_pagespeed`. Księgi wieczyste i pipeline analityczny: `fetch_kw_content` (pobranie treści KW z EKW przez CMD — zużywa limit, przy statusie `processing` wywołaj ponownie), `get_kw_content` (treść z cache, bez kosztu — sprawdzaj najpierw), `analyze_kw` (pełna analiza jednej KW: wyciąg, stan prawny, silnik reguł, właściciele CEIDG/KRS, lokalizacja, przy wniosku także ocena ryzyka), `run_analysis_pipeline` / `get_analysis_pipeline_run` (pipeline wniosku: KW → właściciele → analiza KW → ryzyko). Wszystkie dane klienta: `find_client` (szukaj po nazwisku, e-mailu, telefonie, PESEL, NIP, numerze KW albo id), `get_client_dossier` (pełna teczka: dane, profil, wnioski, nieruchomości, dokumenty, KW, analizy, komunikacja, czat, umowy, KYC/AML, windykacja — zacznij od niej, gdy użytkownik pyta o klienta), `get_client_document` (treść dokumentu: OCR, obraz, link). Umowy pożyczki (silnik klauzul — Ty wypełniasz tylko dane, tekst składa kod): `get_contract_schema` (schemat i zasady — zacznij od niego), `draft_contract` (szkic z profilu klienta, numerów KW i danych od użytkownika; zwraca braki i podgląd; przekazuj zwrócone `umowa` w kolejnych wywołaniach), `generate_contract_docx` (plik .docx z linkiem — dopiero po akceptacji podglądu). Księgowość (administrator/księgowość): `get_accounting_overview` (stan w pigułce — zacznij od niego), `list_accounting_entities` (podmioty wystawiające), `list_sales_invoices` / `get_sales_invoice`, `list_receivables` (należności i wiekowanie), `get_vat_summary` (rejestr VAT za miesiąc/kwartał/rok: należny, naliczony, do zapłaty), `list_accounting_documents` / `get_accounting_document` (rejestr sprzedaży i kosztów z KSeF, XML), `create_sales_invoice` (szkic z pozycjami — kwoty liczy kod; pokaż podgląd), `update_sales_invoice` (tylko szkic), `preview_ksef_invoice` (braki + XML FA(3) bez wysyłki — pokaż przed wystawieniem), `issue_sales_invoice` (numer + wysyłka FA(3) do KSeF — na produkcji faktura o mocy prawnej, tylko po akceptacji; wysyła też faktury wystawione wcześniej bez KSeF), `refresh_ksef_status` (status w KSeF i UPO), `update_accounting_entity` (adres, zwolnienia z VAT, limit, KSeF), `set_sales_invoice_status` (zapłacona/wysłana/anulowana), `add_purchase_document` (faktura kosztowa spoza KSeF), `sync_accounting` (pobranie z KSeF), `get_accounting_sync_status`, `list_individual_sales` / `update_individual_sale_buyer` / `issue_individual_sale_invoice` (rejestr sprzedaży osób fizycznych). Bezpośredni dostęp do bazy Supabase (tylko administrator, rola serwisowa — bez RLS; używaj, gdy narzędzia dziedzinowe nie wystarczają): `supabase_list_tables` (schemat: tabele, kolumny, funkcje RPC — zacznij od niego), `supabase_select` (odczyt dowolnej tabeli z filtrami), `supabase_insert` / `supabase_update` / `supabase_delete` (zapis; update i delete wymagają filtrów i pilnują `max_rows`), `supabase_rpc`, `supabase_storage` (kubełki, pliki, podpisane linki), `supabase_invoke_function` (edge functions). Gdy użytkownik pyta „jak się pozycjonuje strona”, zacznij od `get_search_trend` i `get_top_queries`. Każdy zapis wykonuj tylko na wyraźne polecenie użytkownika; przed wysyłką wiadomości pokaż dokładną treść i poczekaj na potwierdzenie. Widoczność danych ograniczają polityki RLS bazy i role — użytkownik widzi i zmienia tylko to, do czego ma uprawnienia.",
