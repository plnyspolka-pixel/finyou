...
export default defineTool({
  ...
  handler: async ({ id }, ctx: ToolContext) => {
    ...
    const { data: app, error } = await s.from("loan_applications").select("*").eq("id", id).maybeSingle();
    if (error) return fail(error.message);
    if (!app) return fail("Application not found");
    const { data: offers } = await s
      .from("investor_offers")
      .select("id, investor_id, offer_status, proposed_amount, period_months, expected_yearly_yield, estimated_monthly_payment, created_at")
