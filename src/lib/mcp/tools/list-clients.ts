...
export default defineTool({
  name: "list_clients",
  title: "List clients",
  description: "Zwraca listę klientów Finance You (dostęp wg RLS: administrator/operator widzą wszystkich).",
  inputSchema: {
  ...
  handler: async ({ query, limit }, ctx: ToolContext) => {
    ...
    if (query) {
      ...
      q = q.or(`first_name.ilike.%${s2}%,last_name.ilike.%${s2}%,email.ilike.%${s2}%,phone.ilike.%${s2}%`);
    }
