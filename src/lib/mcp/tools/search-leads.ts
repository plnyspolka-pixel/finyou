...
export default defineTool({
  name: "search_leads",
  title: "Search leads",
  description: "Wyszukuje leady po imieniu/nazwisku, emailu lub telefonie. Zwraca do 25 wyników posortowanych najnowsze na górze.",
  inputSchema: {
  ...
  handler: async ({ query, limit }, ctx: ToolContext) => {
    ...
    const { data, error } = await s
      ...
      .or(`first_name.ilike.%${q}%,last_name.ilike.%${q}%,email.ilike.%${q}%,phone_raw.ilike.%${q}%`)
