...
//
// Warstwa dostępu EasyMKW — używana przez fetchAndStoreKw (kw-fetch.server.ts).
//
...
export function easyMkwOrderContent(
  ...
  return call<unknown>("/v1/orders", {
    ...
    body: {
      ...
      kws: [kw],
      // API parsuje to pole przez java.time.Period.parse; przy tym samym
      // startDate i endDate P1D uruchamia dokładnie jedno pobranie.
      period: "P1D",
      reports: [],
      valuations: [],
 ...
 *  UWAGA: parametry page_number/page_size wywalają API (HTTP 500) —
 *  poprawne nazwy to pageNumber/pageSize. */
export function easyMkwJobsForKw(kw: string) {
  return call<{ jobs: EasyMkwJob[]; totalRowCount: number }>(
    `/v1/jobs?kw_number_eq=${encodeURIComponent(kw)}&pageNumber=0&pageSize=50`,
  );
}
