...
export const planSeoTopics = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: settings } = await supabase.from("ai_growth_settings").select("*").limit(1).maybeSingle();
    ...
    const tools = [{
      ...
      function: {
        ...
        parameters: {
          ...
          properties: {
            topics: {
              type: "array", minItems: data.count, maxItems: data.count,
              items: {
                type: "object",
                properties: {
                  title: { type: "string" },
    ...
    const json: any = await callAI(apiKey, model,
    ...
    await supabase.from("ai_growth_action_log").insert({
      module: "seo_content_engine", action: "plan_topics", status: "ok",
      summary: `Zaplanowano ${rows.length} tematów`, payload: { seed: data.seed }, actor: userId,
    });
...
export const generateSeoArticle = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (!apiKey) throw new Error("Brak LOVABLE_API_KEY");
    const { supabase, userId } = context;
    const { data: topic } = await supabase.from("ai_seo_topics").select("*").eq("id", data.topicId).maybeSingle();
      ...
      function: {
        ...
        parameters: {
          ...
          properties: {
            ...
            meta_description: { type: "string", description: "max 160 znaków, z keywordem i CTA" },
            excerpt: { type: "string", description: "1-2 zdania zajawki" },
            content_md: { type: "string", description: "Treść w markdown: 800-1500 słów, H2/H3, listy, FAQ na końcu" },
...
Dodatkowe notatki: ${topic.notes ?? "brak"}`,
    ...
    while (true) {
      ...
      const { data: ex } = await supabase.from("ai_seo_articles").select("id").eq("slug", candidate).maybeSingle();
      if (!ex) { slug = candidate; break; }
      suffix++; if (suffix > 50) throw new Error("Nie mogę nadać unikalnego slug");
    }

    const wordCount = (a.content_md.match(/\S+/g) ?? []).length;
    const status = data.autoPublish ? "published" : "draft";
    const { data: article, error } = await supabase.from("ai_seo_articles").insert({
      topic_id: topic.id,
      slug, title: a.title,
      meta_title: a.meta_title, meta_description: a.meta_description, excerpt: a.excerpt,
      content_md: a.content_md, keywords: a.keywords ?? [],
      primary_keyword: topic.primary_keyword,
      word_count: wordCount, reading_minutes: Math.max(1, Math.round(wordCount / 200)),
      status, cta_url: ctaUrl,
    ...
    await supabase.from("ai_seo_topics").update({ status: status === "published" ? "published" : "generated", article_id: article.id }).eq("id", topic.id);
    await supabase.from("ai_growth_action_log").insert({
      module: "seo_content_engine", action: status === "published" ? "generate_and_publish" : "generate_article",
      status: "ok", summary: `Artykuł: ${a.title}`, payload: { article_id: article.id, slug }, actor: userId,
    });
    return { article };
  });
...
export const setArticleStatus = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((i) => z.object({ id: z.string().uuid(), status: z.enum(["draft", "published", "archived"]) }).parse(i))
  .handler(async ({ data, context }) => {
    const { supabase, userId } = context;
    ...
    if (error) throw new Error(error.message);
    await supabase.from("ai_growth_action_log").insert({
      module: "seo_content_engine", action: `status_${data.status}`, status: "ok",
      summary: `Status artykułu → ${data.status}`, payload: { article_id: data.id }, actor: userId,
    });
    return { ok: true };
  });
